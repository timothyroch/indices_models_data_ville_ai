"""
Research-grade LSTM temporal sequence encoder for Phase 6 urban memory.
...
LSTM hidden and cell states through ``RecurrentSequenceEncoderRun``.
...
``encode_with_state(history, *, parameter_snapshot=None)``
    ...
    canonical final hidden/cell states, state layout, and execution metadata.
...
Packed path
    ...
    7. Execute one ``nn.LSTM`` with exact-zero hidden and cell states.
...
All-zero history
    ...
    and the LSTM kernel are skipped. Sequence output and final hidden/cell
    states are allocated as exact zeros without an artificial autograd anchor.
...
- final backward state is taken from PyTorch's returned ``h_n``/``c_n`` rather
  than inferred from a restored sequence endpoint.
- caller-supplied, learned, streaming, and TBPTT initial states are postponed.
- PyTorch LSTM ``proj_size`` is not supported in Phase 6.
"""
...
LSTM_SEQUENCE_ENCODER_IMPLEMENTATION_VERSION: Final[str] = "0.1"

LSTM_SEQUENCE_ENCODER_COMPONENT_NAME: Final[str] = (
    "lstm_sequence_encoder"
)

LSTM_SEQUENCE_ENCODER_COMPONENT_KIND: Final[str] = (
    "recurrent_temporal_sequence_encoder"
)

LSTM_SEQUENCE_ENCODER_OPERATION_NAME: Final[str] = (
    "encode_temporal_history_with_lstm"
)

LSTM_SEQUENCE_ENCODER_ENCODING_NAME: Final[str] = (
    "lstm_temporal_sequence_encoding"
)

LSTM_SEQUENCE_ENCODER_RUN_NAME: Final[str] = (
    "lstm_sequence_encoder_run"
)

LSTM_SEQUENCE_ENCODER_INITIAL_STATE_POLICY: Final[str] = (
    "exact_zero_hidden_and_cell_v1"
)

LSTM_SEQUENCE_ENCODER_REFERENCE_POLICY: Final[str] = (
    "independent_exact_valid_subsequence_per_nonempty_node_v1"
)

LSTM_SEQUENCE_ENCODER_ALL_ZERO_POLICY: Final[str] = (
    "validate_then_skip_adapter_packing_and_kernel_v1"
)

LSTM_SEQUENCE_ENCODER_OUTPUT_DIRECTION_ORDER: Final[tuple[str, ...]] = (
    "forward",
    "backward",
)

LSTM_SEQUENCE_ENCODER_PYTORCH_PROJECTION_SUPPORTED: Final[bool] = False

LSTM_SEQUENCE_ENCODER_FEATURE_OBSERVATION_MASK_CONSUMED: Final[bool] = False

LSTM_SEQUENCE_ENCODER_TEMPORAL_COORDINATES_CONSUMED: Final[bool] = False

LSTM_SEQUENCE_ENCODER_HAZARD_CONDITIONED: Final[bool] = False

LSTM_SEQUENCE_ENCODER_SCIENTIFIC_INTERPRETATION: Final[str] = (
...
def _validate_lstm_config(
    ...
    if config.cell_kind != RecurrentCellKind.LSTM:
        raise ValueError(
            "LSTMSequenceEncoder requires config.cell_kind='lstm'."
        )
...
class LSTMSequenceEncoder(nn.Module):
    """
    Phase 6 LSTM sequence encoder with packed and exact-reference execution.

    Parameters
    ----------
    config:
        Frozen recurrent encoder configuration with ``cell_kind='lstm'``.
    ...
    def build_parameter_snapshot(
        ...
        *,
        ...
        """
        Explicitly capture the current adapter and LSTM parameter state.
    ...
    def encode_with_state(
        ...
        *,
        ...
        """Encode history and retain canonical final LSTM hidden/cell states."""
    ...
    def _execute_packed(
        ...
        *,
        ...
        _validate_raw_state(
            ...
            name="packed_lstm_final_hidden_state",
        )
        _validate_raw_state(
            execution_cell,
            layout=state_layout,
            ...
            expected_device=source_history.device,
            expected_dtype=source_history.dtype,
            name="packed_lstm_final_cell_state",
        )
        ...
        _validate_raw_sequence(
            ...
            expected_device=source_history.device,
            expected_dtype=source_history.dtype,
            name="packed_lstm_sequence_output",
        )
        ...
        *,
        ...
        for (
            ...
            _validate_raw_state(
                ...
                name=(
                    "reference_lstm_node_hidden_state"
                ),
            )
            _validate_raw_state(
                node_cell,
                ...
                name=(
                    "reference_lstm_node_cell_state"
                ),
            )
        ...
        _validate_raw_sequence(
            ...
            expected_dtype=source_history.dtype,
            name=(
                "reference_lstm_nonempty_source_sequence"
            ),
        )
        _validate_raw_state(
            execution_hidden,
            ...
            expected_dtype=source_history.dtype,
            name=(
                "reference_lstm_nonempty_hidden_state"
            ),
        )
        _validate_raw_state(
            execution_cell,
            ...
            expected_dtype=source_history.dtype,
            name=(
                "reference_lstm_nonempty_cell_state"
            ),
        )
    ...
    def _execute_all_zero_history(
        ...
        *,
        ...
        if final_cell_state is None:
            raise RuntimeError(
                "LSTM all-zero state construction must produce a cell state."
            )

        state_layout.validate_canonical_state(
            final_hidden_state,
            name=(
                "all_zero_lstm_final_hidden_state"
            ),
            node_count=(
                source_history.node_count
            ),
        ...
        state_layout.validate_canonical_state(
            final_cell_state,
            name=(
                "all_zero_lstm_final_cell_state"
            ),
            node_count=(
                source_history.node_count
            ),
    ...
    def _build_run(
        ...
        *,
        ...
        _validate_raw_sequence(
            ...
            expected_device=source_history.device,
            expected_dtype=source_history.dtype,
            name="lstm_encoded_sequence",
        )
        state_layout.validate_canonical_state(
            final_hidden_state,
            name="lstm_final_hidden_state",
            node_count=(
                source_history.node_count
            ),
        )
        ...
        if final_hidden_state.device != source_history.device:
            raise ValueError(
                "LSTM final hidden-state device must match source history."
            )

        if final_hidden_state.dtype != source_history.dtype:
            raise ValueError(
                "LSTM final hidden-state dtype must match source history."
            )

        if final_cell_state.device != source_history.device:
            raise ValueError(
                "LSTM final cell-state device must match source history."
            )

        if final_cell_state.dtype != source_history.dtype:
            raise ValueError(
                "LSTM final cell-state dtype must match source history."
            )
        ...
        computation_provenance = (
            build_recurrent_sequence_computation_provenance(
                ...
                extra_lineage_metadata={
                    ...
                    "pytorch_lstm_projection_supported": False,
                },
                operation_name=(
...
def build_lstm_sequence_encoder(
    config: RecurrentSequenceEncoderConfig,
) -> LSTMSequenceEncoder:
    """Construct a Phase 6 LSTM sequence encoder."""
...
def build_lstm_parameter_snapshot(
    ...
    *,
    ...
    """Explicit functional wrapper for LSTM parameter snapshot creation."""
    ...
    if not isinstance(
        ...
        raise TypeError(
            "encoder must be a LSTMSequenceEncoder."
        )
...
build_lstm_encoder = build_lstm_sequence_encoder
snapshot_lstm_parameters = build_lstm_parameter_snapshot
