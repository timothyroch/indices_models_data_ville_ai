...
Typical run from repository root:
    python urban_graph_benchmark/scripts/10_validate_sovi_against_civil_security_targets.py \
      --sovi-score-col score_normalized_0_1

Optional reporting-only CD name repair:
    python urban_graph_benchmark/scripts/10_validate_sovi_against_civil_security_targets.py \
      --sovi-score-col score_normalized_0_1 \
...
DEFAULT_N_BOOTSTRAPS = 2_000
CONFIDENCE_LEVEL = 0.95
TOPK_DIAGNOSTIC_KS = [10, 25]
...
class Config:
    ...
    cd_boundary_id_col: str
    cd_boundary_name_col: str
...
def parse_args() -> Config:
    ...
    parser.add_argument(
        ...
        help="Number of random-ranking permutations for null significance. Use 0 to disable.",
    )
    parser.add_argument(
        "--n-bootstraps",
        type=int,
        default=DEFAULT_N_BOOTSTRAPS,
        help="Number of CD-level bootstrap samples for confidence intervals. Use 0 to disable.",
    )
    parser.add_argument(
        "--cd-boundaries",
        type=Path,
        default=None,
        help="Optional CD boundary/table path used only to repair missing CD names in outputs.",
    )
    parser.add_argument("--cd-boundary-id-col", default="CDUID")
    parser.add_argument("--cd-boundary-name-col", default="CDNAME")
    args = parser.parse_args()

    if args.n_permutations < 0:
        raise ValueError("--n-permutations must be >= 0")
    if args.n_bootstraps < 0:
        raise ValueError("--n-bootstraps must be >= 0")
    ...
    return Config(
        ...
        cd_boundary_id_col=str(args.cd_boundary_id_col),
        cd_boundary_name_col=str(args.cd_boundary_name_col),
    )
...
def missing_like_name(value: Any) -> bool:
    if pd.isna(value):
        return True
    text = str(value).strip()
    return text == "" or text.lower() in {"none", "nan", "<na>", "null"}


def read_cd_name_lookup(path: Path, id_col: str, name_col: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Read an optional CD ID -> CD name lookup from a boundary/table file.

    This function is used only to repair reporting labels. It must not affect scores,
    targets, rankings, or metrics.
    """
    if path is None:
        return pd.DataFrame(), pd.DataFrame()
    if not path.exists():
        raise FileNotFoundError(f"CD boundary/name lookup file does not exist: {path}")
    ...
    if suffix in {".shp", ".geojson", ".json", ".gpkg"}:
        try:
            ...
            raise RuntimeError(
                "Reading spatial CD lookup files requires geopandas. "
                "Install geopandas or pass a CSV/parquet lookup."
            ) from exc
        lookup = pd.DataFrame(gpd.read_file(path))
    else:
        lookup = read_table(path)

    if id_col not in lookup.columns:
        raise ValueError(f"CD lookup ID column not found: {id_col}. Available columns: {list(lookup.columns)}")
    if name_col not in lookup.columns:
        raise ValueError(f"CD lookup name column not found: {name_col}. Available columns: {list(lookup.columns)}")

    out = lookup[[id_col, name_col]].copy()
    out["_cd_id_norm_lookup"] = out[id_col].map(normalize_id_value)
    out["_cd_name_lookup"] = out[name_col].astype("string")
    out = out.dropna(subset=["_cd_id_norm_lookup"]).drop_duplicates("_cd_id_norm_lookup")
        ...
        [
            {
                ...
                "lookup_name_col": name_col,
                "lookup_rows": int(len(lookup)),
                "unique_lookup_ids": int(out["_cd_id_norm_lookup"].nunique()),
                "missing_lookup_names": int(out["_cd_name_lookup"].isna().sum()),
            }
        ]
    )
    return out[["_cd_id_norm_lookup", "_cd_name_lookup"]], audit
...
def repair_cd_names(
    ...
    *,
    ...
    """Fill missing/None CD names from an optional lookup without changing metrics."""
    out = df.copy()
    final_name_col = name_col
    if final_name_col is None:
        final_name_col = "cd_name"
        if final_name_col not in out.columns:
            out[final_name_col] = pd.NA

    before_missing = int(out[final_name_col].map(missing_like_name).sum()) if final_name_col in out.columns else len(out)
    filled = 0

    if not lookup.empty:
        tmp = out[[id_col]].copy()
        tmp["_cd_id_norm_lookup"] = tmp[id_col].map(normalize_id_value)
        tmp = tmp.merge(lookup, on="_cd_id_norm_lookup", how="left")

        if final_name_col not in out.columns:
            out[final_name_col] = pd.NA
        needs_name = out[final_name_col].map(missing_like_name)
        can_fill = tmp["_cd_name_lookup"].notna()
        fill_mask = needs_name & can_fill
        out.loc[fill_mask, final_name_col] = tmp.loc[fill_mask, "_cd_name_lookup"].astype("string").to_numpy()
        filled = int(fill_mask.sum())

    after_missing = int(out[final_name_col].map(missing_like_name).sum()) if final_name_col in out.columns else len(out)
    audit = pd.DataFrame(
        [
            {
                "cd_id_col": id_col,
                "cd_name_col": final_name_col,
                "rows": int(len(out)),
                "missing_names_before_repair": before_missing,
                "names_filled_from_lookup": filled,
                "missing_names_after_repair": after_missing,
            }
        ]
    )
    return out, final_name_col, audit
...
def metric_value_from_row(row: Mapping[str, Any], metric: str) -> float:
...
def compute_permutation_null(
    ...
    *,
    ...
    """Compare observed SoVI ranking metrics against random score permutations."""
    rows: list[dict[str, Any]] = []
    if n_permutations <= 0:
        return pd.DataFrame(rows)

    rng = np.random.default_rng(random_seed)
    base_score = pd.to_numeric(df[score_col], errors="coerce").to_numpy(dtype=float)

    observed = observed_metrics[observed_metrics["scorer_id"].eq("SoVI_direct")].copy()
    observed_lookup = {
        (str(r["target_id"]), metric): metric_value_from_row(r, metric)
        for _, r in observed.iterrows()
        for metric in ALL_METRICS
        if metric in observed.columns
    }

    for spec in targets:
        target_col = spec["target_column"]
        target_id = spec["target_id"]
        null_values: dict[str, list[float]] = {metric: [] for metric in ALL_METRICS}

        work = df[[id_col, target_col]].copy()
        work["_perm_score"] = base_score

        for _ in range(n_permutations):
            permuted = rng.permutation(base_score)
            work["_perm_score"] = permuted
            metric_row = evaluate_ranking(
                work,
                score_col="_perm_score",
                target_col=target_col,
                id_col=id_col,
            )
            for metric in ALL_METRICS:
                value = metric_value_from_row(metric_row, metric)
                if not math.isnan(value):
                    null_values[metric].append(value)

        for metric, values in null_values.items():
            observed_value = observed_lookup.get((target_id, metric), math.nan)
            arr = np.asarray(values, dtype=float)
            arr = arr[np.isfinite(arr)]
            if arr.size == 0 or math.isnan(observed_value):
                rows.append(
                    {
                        "target_id": target_id,
                        "target_label": spec["target_label"],
                        ...
                        "null_p500": math.nan,
                        "null_p975": math.nan,
                        "empirical_p_value_greater_equal": math.nan,
                        "empirical_p_value_abs_two_sided": math.nan,
                    }
                )
                continue

            # All metrics are interpreted as higher = better. For correlations, also
            # report a two-sided absolute null p-value.
            p_ge = float((1 + np.sum(arr >= observed_value)) / (arr.size + 1))
            p_abs = float((1 + np.sum(np.abs(arr) >= abs(observed_value))) / (arr.size + 1))
            rows.append(
                {
                    "target_id": target_id,
                    "target_label": spec["target_label"],
                    ...
                    "null_std": float(np.std(arr, ddof=1)) if arr.size > 1 else math.nan,
                    "null_p025": float(np.quantile(arr, 0.025)),
                    "null_p500": float(np.quantile(arr, 0.500)),
                    "null_p975": float(np.quantile(arr, 0.975)),
                    "empirical_p_value_greater_equal": p_ge,
                    "empirical_p_value_abs_two_sided": p_abs,
                }
            )
...
def compute_bootstrap_ci(
    ...
    *,
    ...
    """Bootstrap CDs with replacement to quantify metric uncertainty."""
    rows: list[dict[str, Any]] = []
    if n_bootstraps <= 0:
        return pd.DataFrame(rows)

    rng = np.random.default_rng(random_seed + 100_000)
    n = len(df)
    observed = observed_metrics[observed_metrics["scorer_id"].eq("SoVI_direct")].copy()
    observed_lookup = {
        (str(r["target_id"]), metric): metric_value_from_row(r, metric)
    ...
    for spec in targets:
        target_col = spec["target_column"]
        target_id = spec["target_id"]
        boot_values: dict[str, list[float]] = {metric: [] for metric in ALL_METRICS}

        for _ in range(n_bootstraps):
            sample_idx = rng.integers(0, n, size=n)
            boot = df.iloc[sample_idx].reset_index(drop=True).copy()
            # Use row-level bootstrap IDs so top-k metrics behave on the resampled sample.
            boot["_bootstrap_row_id"] = [f"boot_{i}" for i in range(len(boot))]
            metric_row = evaluate_ranking(
                boot,
                score_col=score_col,
                target_col=target_col,
                id_col="_bootstrap_row_id",
            )
            for metric in ALL_METRICS:
                value = metric_value_from_row(metric_row, metric)
                if not math.isnan(value):
        ...
        for metric, values in boot_values.items():
            ...
            arr = arr[np.isfinite(arr)]
            if arr.size == 0:
                q025 = q500 = q975 = mean = std = math.nan
            else:
...
def independent_top_overlap(score: pd.Series, target: pd.Series, ids: pd.Series, k: int) -> float:
    tmp = pd.DataFrame(
        {
            "score": pd.to_numeric(score, errors="coerce"),
            "target": pd.to_numeric(target, errors="coerce"),
            "id": ids.astype("string"),
        }
    ).dropna(subset=["score", "target"])
    if tmp.empty:
        return math.nan
    k_eff = min(k, len(tmp))
    pred = set(tmp.sort_values(["score", "id"], ascending=[False, True], kind="mergesort").head(k_eff)["id"])
    truth = set(tmp.sort_values(["target", "id"], ascending=[False, True], kind="mergesort").head(k_eff)["id"])
    return float(len(pred & truth) / k_eff)
...
def compute_metric_audit(
    ...
    *,
    ...
    """Recompute headline metrics through independent/simple implementations."""
    rows: list[dict[str, Any]] = []
    observed = observed_metrics[observed_metrics["scorer_id"].eq("SoVI_direct")].copy()
    ...
    for spec in targets:
        ...
        obs_row = observed[observed["target_id"].eq(spec["target_id"])]
        obs = obs_row.iloc[0].to_dict() if not obs_row.empty else {}

        audit_values = {
            "spearman": float(score_v.corr(target_v, method="spearman")) if len(score_v) >= 3 else math.nan,
            "kendall": float(score_v.corr(target_v, method="kendall")) if len(score_v) >= 3 else math.nan,
            "top10_overlap_rate": independent_top_overlap(score_v, target_v, ids_v, 10),
            "top25_overlap_rate": independent_top_overlap(score_v, target_v, ids_v, 25),
        }
        for metric, audit_value in audit_values.items():
            main_value = metric_value_from_row(obs, metric)
            diff = abs(main_value - audit_value) if np.isfinite(main_value) and np.isfinite(audit_value) else math.nan
            rows.append(
                {
                    "target_id": spec["target_id"],
                    "target_label": spec["target_label"],
                    ...
                    "abs_diff": diff,
                    "passes_tolerance": bool(diff <= 1e-9) if not math.isnan(diff) else False,
                }
            )
...
def build_topk_diagnostics(
    rankings: pd.DataFrame,
    *,
    ks: list[int] | None = None,
) -> pd.DataFrame:
    if ks is None:
        ks = TOPK_DIAGNOSTIC_KS
    rows: list[dict[str, Any]] = []
    if rankings.empty:
        return pd.DataFrame(rows)

    for target_id, sub in rankings.groupby("target_id", sort=False):
        sub = sub.copy()
        target_label = sub["target_label"].iloc[0]
        target_column = sub["target_column"].iloc[0]
        for k in ks:
            sovi_top = set(sub.loc[sub["sovi_rank_desc"] <= k, "cd_id_norm"].astype(str))
            target_top = set(sub.loc[sub["target_rank_desc"] <= k, "cd_id_norm"].astype(str))
            union = sorted(sovi_top | target_top)
            for cd_id in union:
                r = sub[sub["cd_id_norm"].astype(str).eq(cd_id)].iloc[0]
                in_sovi = cd_id in sovi_top
                in_target = cd_id in target_top
                if in_sovi and in_target:
                    diagnostic_class = "intersection"
                elif in_sovi:
                    diagnostic_class = "sovi_only"
                else:
                    diagnostic_class = "target_only"
                rows.append(
                    {
                        "target_id": target_id,
                        ...
                        "k": k,
                        "cd_id_norm": cd_id,
                        "cd_name": r.get("cd_name"),
                        "diagnostic_class": diagnostic_class,
                        "in_sovi_topk": bool(in_sovi),
                        "in_target_topk": bool(in_target),
                        "sovi_score": r.get("sovi_score"),
                        "target_value": r.get("target_value"),
                        "sovi_rank_desc": r.get("sovi_rank_desc"),
                        "target_rank_desc": r.get("target_rank_desc"),
                    }
                )
...
def run_b1_sovi_direct_validation(config: Config) -> dict[str, Any]:
    """Run B1 direct SoVI validation and return the metadata dictionary."""
    ...
    # Optional reporting-only CD name repair. This must happen before rankings are
    # written, but it must never change metrics, target values, or scores.
    cd_lookup = pd.DataFrame()
    cd_lookup_audit = pd.DataFrame()
    if config.cd_boundaries is not None:
        cd_lookup, cd_lookup_audit = read_cd_name_lookup(
            config.cd_boundaries,
            id_col=config.cd_boundary_id_col,
            name_col=config.cd_boundary_name_col,
        )
    df, name_col, cd_name_repair_audit = repair_cd_names(
        df,
        id_col=id_col,
        name_col=name_col,
        lookup=cd_lookup,
    ...
    # Hardening outputs: these do not modify benchmark metrics. They quantify
    # uncertainty, compare against a large random-ranking null, and expose top-k
    # mismatch cases for interpretation.
    permutation_null = compute_permutation_null(
        df_eval,
        targets=targets,
        score_col="_score_sovi_direct",
        id_col=id_col,
    ...
    metric_audit = compute_metric_audit(
        df_eval,
        targets=targets,
        score_col="_score_sovi_direct",
        id_col=id_col,
    ...
    topk_diagnostics = build_topk_diagnostics(rankings, ks=TOPK_DIAGNOSTIC_KS)
    ...
    permutation_null_path = config.output_dir / "sovi_civil_security_validation_permutation_null.csv"
    bootstrap_ci_path = config.output_dir / "sovi_civil_security_validation_bootstrap_ci.csv"
    topk_diagnostics_path = config.output_dir / "sovi_civil_security_validation_topk_diagnostics.csv"
    metric_audit_path = config.output_dir / "sovi_civil_security_validation_metric_audit.csv"
    cd_name_repair_audit_path = config.output_dir / "sovi_civil_security_validation_cd_name_repair_audit.csv"
    cd_name_lookup_audit_path = config.output_dir / "sovi_civil_security_validation_cd_name_lookup_audit.csv"
    ...
    topk_diagnostics.to_csv(topk_diagnostics_path, index=False)
    metric_audit.to_csv(metric_audit_path, index=False)
    cd_name_repair_audit.to_csv(cd_name_repair_audit_path, index=False)
    cd_lookup_audit.to_csv(cd_name_lookup_audit_path, index=False)
    ...
    metadata = {
        "status": "completed",
        "benchmark_type": "SoVI direct external-validation benchmark, hardened with uncertainty/null diagnostics",
        "scientific_question": (
            "Do Québec CDs with higher SoVI-like vulnerability scores tend to rank higher "
        ...
        "constraints": {
            ...
            "hardening_changes_result_driven": False,
        },
        "input_targets_path": str(config.targets_path),
        "output_dir": str(config.output_dir),
        "cd_id_col": id_col,
        "cd_name_col": name_col,
        "cd_boundaries_for_name_repair": str(config.cd_boundaries) if config.cd_boundaries is not None else None,
        "cd_boundary_id_col": config.cd_boundary_id_col,
        "cd_boundary_name_col": config.cd_boundary_name_col,
        ...
        "metric_notes": {
            ...
            "permutation_null": "Randomly permutes the SoVI score across CDs. Higher-is-better empirical p-values are reported for all metrics; absolute two-sided p-values are most relevant for correlations.",
            "bootstrap_ci": "Bootstrap resamples CDs with replacement. Top-k bootstrap metrics use row-level bootstrap IDs to avoid duplicate-ID set artifacts.",
            "topk_diagnostics": "Shows top-k intersections, SoVI-only CDs, and target-only CDs for top-10 and top-25.",
        },
        "primary_target": {
            "target_id": "B1a",
            "target_column": "event_count_2021_2025_all",
        ...
        "outputs": {
            ...
            "permutation_null": str(permutation_null_path),
            "bootstrap_ci": str(bootstrap_ci_path),
            "topk_diagnostics": str(topk_diagnostics_path),
            "metric_audit": str(metric_audit_path),
            "cd_name_repair_audit": str(cd_name_repair_audit_path),
            "cd_name_lookup_audit": str(cd_name_lookup_audit_path),
        },
    }
    ...
    print(f"Bootstraps: {config.n_bootstraps:,}")
    print()
    print("Outputs:")
    ...
    print(f"  permutation null: {permutation_null_path}")
    print(f"  bootstrap CI: {bootstrap_ci_path}")
    print(f"  top-k diagnostics: {topk_diagnostics_path}")
    print(f"  metric audit: {metric_audit_path}")
    print()
    if primary_row:
...
def main() -> None:
    config = parse_args()
    run_b1_sovi_direct_validation(config)
