#!/usr/bin/env python3
"""
A3 feature-parity tabular baselines WITH model-behavior plots.

This module is intentionally separate from ``a3_tabular_feature_parity.py`` so
the original A3 outputs remain safe. It imports and reuses the original A3
...
Save as:
    urban_graph_benchmark/src/ville_hgnn/baselines/a3_tabular_feature_parity_with_plots.py
...
from ville_hgnn.baselines import a3_tabular_feature_parity as base
...
STAGE_SLUG = "A3_feature_parity_tabular_with_plots"
MODEL_STAGE = "A3_feature_parity_tabular_with_plots"
...
def selected_reference_models(
    ...
    if not strict_primary.empty:
        ...
        refs["best_strict_primary"] = str(row["model_name"])
    ...
    if not strict_overall.empty:
        ...
        refs["best_strict_overall"] = str(row["model_name"])
    ...
    if not retrospective_primary.empty:
        ...
        refs["best_retrospective_primary"] = str(row["model_name"])
    ...
    if not static_primary.empty:
        ...
        refs["best_static_primary"] = str(row["model_name"])
...
def preferred_surface_pairs(fitted: base.FittedTabularModel) -> list[tuple[str, str]]:
    """Choose meaningful 2D surface feature pairs when present."""

    features = set(fitted.feature_columns)

    candidates = [
        ("static_spatial__tract_centroid_lon", "static_spatial__tract_centroid_lat"),
        ("target_train_summary__mean", "reporting_history__total_311_count_non_water_drainage_lag_1"),
        ("target_train_summary__mean", "svi_primary__svi_percentile"),
        ("target_history__water_drainage_count_lag_1", "reporting_history__total_311_count_non_water_drainage_lag_1"),
        ("svi_primary__svi_percentile", "static__log1p_population_density"),
        ("reporting_retro__log1p_total_311_count_non_water_drainage", "target_train_summary__mean"),
    ]
...
def plot_surface_heatmap_and_3d(
    ...
    """Plot heatmap and 3D response surface for two features."""
    ...
    im = ax.contourf(X, Y, Z, levels=22, cmap="viridis")
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label("Predicted water/drainage count")
    ax.set_title(f"2D response surface — {clean_label(fitted.model_name, 54)}")
    ax.set_xlabel(clean_label(feature_x, 52))
    ax.set_ylabel(clean_label(feature_y, 52))
    ...
    # 3D surface, closer to the kind of plot the user showed.
    fig = plt.figure(figsize=(8.2, 6.2))
    ax = fig.add_subplot(111, projection="3d")
    surf = ax.plot_surface(X, Y, Z, cmap="viridis", linewidth=0.15, antialiased=True, alpha=0.92)
    ax.contour(X, Y, Z, zdir="z", offset=float(np.nanmin(Z)), cmap="viridis", alpha=0.45)
    ax.set_title(f"3D response surface — {clean_label(fitted.model_name, 50)}")
    ax.set_xlabel(clean_label(feature_x, 42), labelpad=8)
    ax.set_ylabel(clean_label(feature_y, 42), labelpad=8)
    ax.set_zlabel("Predicted count", labelpad=8)
    fig.colorbar(surf, shrink=0.6, aspect=18, pad=0.08)
...
def generate_plots(
    *,
    ...
    # Deep model-behavior plots for the best primary strict model.
    reference_name = refs.get("best_strict_primary") or refs.get("best_strict_overall")
    ...
    if reference_model is not None:
        ...
        for i, (fx, fy) in enumerate(preferred_surface_pairs(reference_model), start=1):
...
def render_plot_index(plot_paths: Mapping[str, str], output_dir: Path) -> str:
    ...
    lines.append(
        "These figures are generated by `a3_tabular_feature_parity_with_plots.py`. "
...
def render_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# A3 Feature-Parity Tabular Baselines with Plots — Montréal 311 Water/Drainage v0\n")
    ...
    return to_jsonable(
        {
            ...
            "model_stage": MODEL_STAGE,
            "model_name": "A3_feature_parity_tabular_with_plots_suite",
            "split_scheme": split_scheme,
            ...
            "notes": (
                ...
                "A3_feature_parity_tabular output folder."
            ),
        }
    )


def run_a3_tabular_feature_parity_with_plots(
    ...
    frame, feature_groups, feature_lineage = base.prepare_feature_frame(
        frame,
        split_scheme,
        svi_specs,
    )
...
def a3_with_plots_brief(result: Mapping[str, Any]) -> str:
    ...
    return (
        "A3 feature-parity tabular baselines with plots completed.\n"
...
def main() -> None:
    ...
    parser.add_argument("--surface-grid-size", type=int, default=35)
    ...
    result = run_a3_tabular_feature_parity_with_plots(
...
__all__ = [
    ...
    "a3_with_plots_brief",
    "generate_plots",
    "run_a3_tabular_feature_parity_with_plots",
]
