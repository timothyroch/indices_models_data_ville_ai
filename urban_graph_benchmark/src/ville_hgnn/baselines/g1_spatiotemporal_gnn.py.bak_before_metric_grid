...
MONITOR_HIGHER_IS_BETTER = {
    ...
    "validation_mae": False,
    "validation_rmse": False,
    "validation_spearman": True,
    "validation_ndcg_at_100": True,
    "validation_top_10pct_overlap_rate": True,
}

MONITOR_METRIC_TO_AUDIT_COLUMN = {
    "validation_loss": "best_validation_loss",
    "validation_mae": "validation_mae",
    "validation_rmse": "validation_rmse",
    "validation_spearman": "validation_spearman",
    "validation_ndcg_at_100": "validation_ndcg_at_100",
    "validation_top_10pct_overlap_rate": "validation_top_10pct_overlap_rate",
}
...
class TrialSpec:
    ...
    def model_name(self, train_config: TrainConfig) -> str:
        """Stable, collision-safe model name used in metrics and artifacts."""

        dropout_tag = f"{train_config.dropout:g}".replace(".", "p").replace("-", "m")
        lr_tag = f"{train_config.learning_rate:g}".replace(".", "p").replace("-", "m")
        wd_tag = f"{train_config.weight_decay:g}".replace(".", "p").replace("-", "m")
        residual_tag = "res" if train_config.residual else "nores"
        norm_tag = str(train_config.normalization).lower()
        combine_tag = str(train_config.relation_combine).lower()
        backend_tag = str(train_config.backend).lower()

        return (
            f"G1__{self.split_scheme}__{self.feature_regime}__"
            f"{self.edge_regime}__{self.edge_mask_regime}__"
            f"h{train_config.hidden_dim}_L{train_config.num_layers}_"
            f"do{dropout_tag}_{norm_tag}_{residual_tag}_"
            f"rel{combine_tag}_{backend_tag}_lr{lr_tag}_wd{wd_tag}_"
            f"seed{self.seed}"
        )
...
class TrialResult:
    ...
    best_validation_ndcg_at_100: float
    best_validation_top_10pct_overlap_rate: float
    best_monitor_metric: str
...
def build_edge_tensors_for_trial(
    ...
    for edge_type, part in edge_table.groupby("edge_type", sort=True):
        edge_index = part[["source_node_id", "target_node_id"]].to_numpy(dtype=np.int64).T.copy()
        edge_weight = pd.to_numeric(part["edge_weight"], errors="coerce").fillna(1.0).to_numpy(dtype=np.float32).copy()
...
def validate_monitor_metric(monitor_metric: str) -> None:
    """Validate a checkpoint/model-selection monitor metric."""

    if monitor_metric not in MONITOR_HIGHER_IS_BETTER:
        raise G1BaselineError(
            f"Unsupported monitor metric {monitor_metric!r}. "
            f"Supported: {sorted(MONITOR_HIGHER_IS_BETTER)}"
        )


def monitor_improved(
    value: float,
    best_value: float,
    *,
    ...
    """Return whether a monitor value improved relative to the current best."""
...
def initial_best_monitor_value(monitor_metric: str) -> float:
    """Initial best value for a monitor metric."""

    validate_monitor_metric(monitor_metric)
    return -float("inf") if MONITOR_HIGHER_IS_BETTER[monitor_metric] else float("inf")
...
def evaluate_epoch_metrics(
    ...
    *,
    ...
    """Return validation loss, count metrics, and ranking metrics."""
    ...
    with torch.no_grad():
        ...
        pred_count = invert_log_count_prediction(out["log_count"], count_min=count_min)

    mask_np = val_mask.detach().cpu().numpy().astype(bool)
    y_np = y.detach().cpu().numpy().astype(np.float32)
    pred_np = pred_count.detach().cpu().numpy().astype(np.float32)

    y_val = y_np[mask_np]
    pred_val = pred_np[mask_np]

    return {
        "validation_loss": float(val_loss.detach().cpu()),
        "validation_mae": mae(y_val, pred_val),
        "validation_rmse": rmse(y_val, pred_val),
        "validation_spearman": spearman_corr(y_val, pred_val),
        "validation_ndcg_at_100": ndcg_at_k(y_val, pred_val, k=100),
        "validation_top_10pct_overlap_rate": top_fraction_overlap(y_val, pred_val, fraction=0.10),
    }
...
def evaluate_epoch_monitor(
    ...
    *,
    ...
    """Backward-compatible wrapper returning validation log-loss and count MAE."""

    metrics = evaluate_epoch_metrics(
        model,
        x,
        y,
        val_mask,
    ...
    return metrics["validation_loss"], metrics["validation_mae"]
...
def train_one_trial(
    ...
    validate_monitor_metric(train_config.monitor_metric)
    monitor_higher_is_better = MONITOR_HIGHER_IS_BETTER[train_config.monitor_metric]
    ...
    best_val_spearman = float("nan")
    best_val_ndcg_at_100 = float("nan")
    best_val_top_10pct_overlap_rate = float("nan")
    best_monitor = initial_best_monitor_value(train_config.monitor_metric)
    ...
    for epoch in range(1, train_config.max_epochs + 1):
        ...
        val_mae = val_metrics["validation_mae"]
        val_rmse = val_metrics["validation_rmse"]
        val_spearman = val_metrics["validation_spearman"]
        val_ndcg_at_100 = val_metrics["validation_ndcg_at_100"]
        val_top_10pct_overlap_rate = val_metrics["validation_top_10pct_overlap_rate"]
        ...
        monitor = val_metrics[train_config.monitor_metric]
        ...
        if improved:
            ...
            best_val_ndcg_at_100 = val_ndcg_at_100
            best_val_top_10pct_overlap_rate = val_top_10pct_overlap_rate
            best_epoch = epoch
        ...
        training_rows.append(
            {
                ...
                "validation_rmse_count": val_rmse,
                "validation_spearman": val_spearman,
                "validation_ndcg_at_100": val_ndcg_at_100,
                "validation_top_10pct_overlap_rate": val_top_10pct_overlap_rate,
                "monitor_metric": train_config.monitor_metric,
                "monitor_higher_is_better": monitor_higher_is_better,
                "monitor_value": monitor,
                "best_monitor_value_so_far": best_monitor,
                "best_validation_mae_so_far": best_val_mae,
                "best_validation_rmse_so_far": best_val_rmse,
                "best_validation_spearman_so_far": best_val_spearman,
                "best_validation_ndcg_at_100_so_far": best_val_ndcg_at_100,
                "best_validation_top_10pct_overlap_rate_so_far": best_val_top_10pct_overlap_rate,
    ...
    if train_config.save_checkpoints:
        ...
        save_model_checkpoint(
            ...
            optimizer=optimizer,
            epoch=best_epoch,
            validation_metric=best_monitor,
            extra={
                ...
                "best_validation_metrics": {
                    ...
                    "validation_spearman": best_val_spearman,
                    "validation_ndcg_at_100": best_val_ndcg_at_100,
                    "validation_top_10pct_overlap_rate": best_val_top_10pct_overlap_rate,
                },
    ...
    result = TrialResult(
        ...
        best_validation_spearman=float(best_val_spearman),
        best_validation_ndcg_at_100=float(best_val_ndcg_at_100),
        best_validation_top_10pct_overlap_rate=float(best_val_top_10pct_overlap_rate),
        best_monitor_metric=train_config.monitor_metric,
        best_monitor_value=float(best_monitor),
        monitor_higher_is_better=bool(monitor_higher_is_better),
...
def build_model_selection_audit(
    ...
    *,
    ...
    The selected checkpoint for each trial is produced during training according
    to ``selection_metric`` / ``TrainConfig.monitor_metric``. This audit then
    selects trial representatives using the same validation-only metric across
    trials. Test metrics are joined only for reporting.
    """

    if trial_audit.empty:
        return pd.DataFrame()

    validate_monitor_metric(selection_metric)
    higher_is_better = MONITOR_HIGHER_IS_BETTER[selection_metric]
    selection_column = MONITOR_METRIC_TO_AUDIT_COLUMN[selection_metric]
    ...
    for _, trial in trial_audit.iterrows():
        ...
        row["validation_loss"] = float(row.get("best_validation_loss", np.nan))
        ...
        row["validation_mean_poisson_deviance"] = metric_lookup(
            metrics, model_name, "validation", "count__mean_poisson_deviance"
        )
        ...
        row["test_mean_poisson_deviance"] = metric_lookup(
            metrics, model_name, "test", "count__mean_poisson_deviance"
        )
        ...
        row["selection_metric_column"] = selection_column
        row["selection_metric_higher_is_better"] = bool(higher_is_better)
        row["selection_metric_value"] = row.get(selection_column, np.nan)
        rows.append(row)
    ...
    audit["selection_metric_value_num"] = pd.to_numeric(
        audit["selection_metric_value"],
        errors="coerce",
    )

    valid = audit[
        (audit["status"].astype(str) == "completed")
        & audit["selection_metric_value_num"].notna()
    ].copy()

    if not valid.empty:
        for split, group in valid.groupby("split_scheme"):
            best_idx = group.sort_values(
                "selection_metric_value_num",
    ...
    sort_cols = ["split_scheme", "selection_metric_value_num"]
...
def render_report(
    *,
    ...
    lines.append(
        f"Checkpointing and validation selection monitor: `{train_config.monitor_metric}` "
        f"({'higher is better' if MONITOR_HIGHER_IS_BETTER[train_config.monitor_metric] else 'lower is better'}).\n"
    )
    lines.append("```json")
    ...
    summary_cols = [
        ...
        "edge_mask_regime", "best_epoch", "best_monitor_metric", "best_monitor_value",
        "best_validation_mae", "best_validation_ndcg_at_100", "n_edges_used",
    ...
    lines.append(
        f"Trial representatives are selected using `{train_config.monitor_metric}` on validation nodes only. "
...
def run_g1_spatiotemporal_gnn(
    *,
    ...
    for i, spec in enumerate(trial_specs, start=1):
        ...
        try:
            ...
            print(
                ...
                f"monitor={result.best_monitor_metric}={result.best_monitor_value:.4f}, "
            ...
            failure = TrialResult(
                ...
                best_validation_spearman=float("nan"),
                best_validation_ndcg_at_100=float("nan"),
                best_validation_top_10pct_overlap_rate=float("nan"),
                best_monitor_metric=train_config.monitor_metric,
                best_monitor_value=float("nan"),
                monitor_higher_is_better=bool(MONITOR_HIGHER_IS_BETTER.get(train_config.monitor_metric, False)),
                elapsed_seconds=0.0,
                n_parameters=0,
                n_edges_used=0,
                n_relations_used=0,
    ...
    model_selection = build_model_selection_audit(
        metrics_df,
        trial_audit,
        selection_metric=train_config.monitor_metric,
    )
    ...
    metadata = {
        ...
        "train_config": asdict(train_config),
        "monitor_metric": train_config.monitor_metric,
        "monitor_higher_is_better": bool(MONITOR_HIGHER_IS_BETTER[train_config.monitor_metric]),
        "selection_metric": train_config.monitor_metric,
...
def g1_brief(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Monitor metric: {result.get('monitor_metric')}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--monitor-metric", default="validation_mae", choices=list(VALIDATION_MONITOR_METRICS))
