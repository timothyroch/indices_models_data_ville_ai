...
# ============================================================
# Clean Census Division Labour Force Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division labour-force feature table from the
...
#   The raw Statistics Canada Census Profile CD file is best read as cp1252.
#   The existing cleaned CD base frame is likely UTF-8.
#
#   Do NOT use the same encoding priority for both, otherwise the cleaned
#   base geography names can become mojibake, e.g.:
#       La CÃ´te-de-GaspÃ©
#
# Run from data/:
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_labour_force_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_labour_force_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_labour_force_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_labour_force_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_labour_force_summary_2021.csv"
...
# Raw StatCan CD Census Profile file.
...
# Existing cleaned files produced by our own pipeline.
...
LABOUR_TARGETS = [
    {
        ...
        "expected_characteristic_name": "Unemployment rate",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "total_rate",
        "sovi_role": "direct_or_strong_proxy",
        "notes": "Uses total unemployment rate from the Census Profile labour-force section.",
    },
    {
        ...
        "expected_characteristic_name": "Participation rate",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "total_rate",
        "sovi_role": "direct_or_strong_proxy",
        "notes": "Uses total participation rate from the Census Profile labour-force section.",
    },
    {
        ...
        "unit": "percent",
        "source_value_type": "women_plus_rate",
        "sovi_role": "sex_specific_rate_from_same_characteristic",
        "notes": "Uses the Women+ rate column from the Participation rate characteristic.",
    },
]
...
def detect_encoding(path: Path, encodings: list[str]) -> str:
    """
    Try reading a larger sample than nrows=5.

    nrows=5 can be misleading because the first few rows may be ASCII-only even
    when later rows contain accented characters.
    """
    last_error = None
...
def choose_display_name(row: pd.Series) -> str:
    """
    Prefer the existing base name when it is clean.
    Fall back to the Census Profile name if the base name looks mojibaked.
    """
...
target_characteristic_ids = sorted({target["characteristic_id"] for target in LABOUR_TARGETS})

print("\nCleaning Census Division Labour Force Features 2021")
...
if source_long.empty:
    raise ValueError("No target labour-force rows were extracted from the raw Census Profile file.")
...
if not not_ready.empty:
    raise ValueError(
        "One or more labour-force variables failed validation:\n"
...
clean["labour_force_feature_count"] = len(LABOUR_TARGETS)
clean["labour_force_missing_count"] = clean[
    [target["canonical_variable"] for target in LABOUR_TARGETS]
].isna().sum(axis=1)

clean["labour_force_complete"] = clean["labour_force_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, labour-force section"
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in LABOUR_TARGETS),
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["labour_force_complete"].all()),
    },
    {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION LABOUR FORCE FEATURES 2021")
print("=" * 72)
