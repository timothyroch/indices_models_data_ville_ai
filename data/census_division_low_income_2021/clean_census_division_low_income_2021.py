...
# ============================================================
# Clean Census Division Low-Income Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division low-income feature table from the
...
#
#   PCTPOV90:
#       Original SoVI concept: percent persons in poverty.
#       Local Canadian 2021 proxy:
...
#
#       Rationale:
#           The Canadian Census Profile does not use the same U.S. poverty
#           definition as the original SoVI. LIM-AT prevalence is used as the
#           default Canadian low-income / poverty proxy.
#
...
#
# Validated by:
#   census_division_low_income_2021/inspect_census_division_low_income_2021.py
#
# Run from data/:
...
SECTION_DIR = DATA_DIR / "census_division_low_income_2021"
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_low_income_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_low_income_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_low_income_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_low_income_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_low_income_summary_2021.csv"
...
LOW_INCOME_TARGETS = [
    {
        ...
        "description": "Percent population in poverty / low-income proxy",
        "characteristic_id": "345",
        ...
        "unit": "percent",
        "source_value_type": "lim_at_prevalence_rate",
        "sovi_role": "canadian_low_income_proxy_for_poverty",
        "preferred": True,
        "methodological_choice": (
            "Original SoVI uses a U.S. poverty variable. This local Canadian 2021 proxy "
            "uses the prevalence of low income based on the Low-income measure, after tax "
            "(LIM-AT), as the default poverty/low-income proxy."
        ),
        "notes": (
            "Main SoVI-compatible low-income proxy. Values are published Census Profile "
            "percent rates."
        ),
    },
    {
        "original_code": "PCTPOV90_ALT",
        "canonical_variable": "pct_low_income_lico_at",
        "description": "Alternative low-income proxy using LICO-AT",
        "characteristic_id": "360",
        "expected_characteristic_name": (
        ...
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "lico_at_prevalence_rate",
        "sovi_role": "alternative_canadian_low_income_proxy_for_poverty",
        "preferred": False,
        "methodological_choice": (
            "Kept as an alternative audit variable. The default SoVI input should use "
            "pct_poverty_or_low_income from LIM-AT unless a later methodology document "
            "explicitly chooses LICO-AT."
        ),
        "notes": (
            "Alternative low-income measure. Included for audit and sensitivity analysis, "
            "not as the default SoVI poverty variable."
        ),
    },
]
...
target_characteristic_ids = sorted({target["characteristic_id"] for target in LOW_INCOME_TARGETS})

print("\nCleaning Census Division Low-Income Features 2021")
...
for chunk_idx, chunk in enumerate(
    ...
    for target in LOW_INCOME_TARGETS:
...
if source_long.empty:
    raise ValueError("No target low-income rows were extracted from the raw Census Profile file.")
...
if not not_ready.empty:
    raise ValueError(
        "One or more low-income variables failed validation:\n"
...
# Helpful explicit aliases for readability and continuity with previous CT-level work.
clean["pct_low_income_lim_at"] = clean["pct_poverty_or_low_income"]
clean["pct_low_income_lico_at"] = clean["pct_low_income_lico_at"]

clean["low_income_feature_count"] = len(LOW_INCOME_TARGETS)
clean["low_income_missing_count"] = clean[
    [target["canonical_variable"] for target in LOW_INCOME_TARGETS]
].isna().sum(axis=1)

clean["low_income_complete"] = clean["low_income_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, low-income section"
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in LOW_INCOME_TARGETS),
    },
    {
        "metric": "preferred_main_variable",
        "value": "pct_poverty_or_low_income",
    },
    {
        "metric": "alternative_audit_variable",
        "value": "pct_low_income_lico_at",
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["low_income_complete"].all()),
    },
    {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION LOW-INCOME FEATURES 2021")
print("=" * 72)
...
for target in LOW_INCOME_TARGETS:
    var = target["canonical_variable"]
    preferred_label = "preferred" if target["preferred"] else "alternative"
    print(
