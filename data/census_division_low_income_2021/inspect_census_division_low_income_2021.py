...
# ============================================================
# Inspect Census Division Low-Income Features 2021
# ============================================================
#
# Purpose:
#   Inspect the 2021 Census Profile at census-division level for the
#   low-income / poverty-proxy variable needed by the SoVI-like
...
#   after tax (LIM-AT)
#
# This script does NOT create the final clean low-income table.
...
OUTPUT_FILE_INVENTORY = OUTPUT_DIR / "low_income_file_inventory_2021.csv"
OUTPUT_CD_INVENTORY = OUTPUT_DIR / "low_income_quebec_cd_inventory_2021.csv"
OUTPUT_BASE_JOIN = OUTPUT_DIR / "low_income_quebec_cd_base_join_check_2021.csv"
OUTPUT_CANDIDATE_CHARACTERISTICS = OUTPUT_DIR / "low_income_candidate_characteristics_2021.csv"
OUTPUT_TARGET_SUMMARY = OUTPUT_DIR / "low_income_target_characteristic_summary_2021.csv"
OUTPUT_TARGET_VALUES_LONG = OUTPUT_DIR / "low_income_target_values_long_2021.csv"
OUTPUT_TARGET_VALUES_WIDE = OUTPUT_DIR / "low_income_target_values_wide_2021.csv"
OUTPUT_SYMBOL_COUNTS = OUTPUT_DIR / "low_income_symbol_counts_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "low_income_inspection_summary_2021.csv"
...
LOW_INCOME_CONTEXT_IDS = {
    # LIM low-income status block
    "335",
    "336",
    "337",
    "338",
...
LOW_INCOME_TARGETS = [
    {
        ...
        "candidate_output_alias": "pct_low_income_lim_at",
        "target_family": "low_income_lim_at",
        "expected_characteristic_id": "345",
        "expected_characteristic_name_contains": "prevalence of low income based on the low-income measure",
        ...
        "notes": (
            ...
            "(LIM-AT), as the Canadian Census Profile proxy for the original SoVI poverty variable."
        ),
    },
    {
        "original_code": "PCTPOV90",
        "canonical_variable": "pct_poverty_or_low_income_lico_at_candidate",
        "description": "Alternative low-income proxy using LICO-AT",
        "candidate_output_alias": "pct_low_income_lico_at",
        "target_family": "low_income_lico_at",
        "expected_characteristic_id": "360",
        "expected_characteristic_name_contains": "prevalence of low income based on the low-income cut-offs",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        ...
        "preferred": False,
        "notes": (
            "Alternative candidate: prevalence of low income based on the Low-income cut-offs, "
            "after tax (LICO-AT). Kept for audit/review, but LIM-AT is expected to be the default."
        ),
    },
]
...
def is_low_income_candidate(row: pd.Series) -> bool:
    char_id = str(row.get("CHARACTERISTIC_ID", "")).strip()
    name = normalize_text(row.get("CHARACTERISTIC_NAME", ""))

    id_match = char_id in LOW_INCOME_CONTEXT_IDS
    keyword_match = any(keyword.lower() in name for keyword in LOW_INCOME_KEYWORDS_ANY)

    return id_match or keyword_match
...
print("\nCensus Profile CD low-income inspection")
print("Raw file:", RAW_CSV.relative_to(DATA_DIR))
print("Raw encoding selected:", raw_encoding)
print("Target characteristic IDs:", sorted({target["expected_characteristic_id"] for target in LOW_INCOME_TARGETS}))
...
for chunk_idx, chunk in enumerate(
    ...
    candidate_mask = qc.apply(is_low_income_candidate, axis=1)
    candidates = qc.loc[candidate_mask].copy()

    if not candidates.empty:
        candidate_rows.append(candidates)

    target_ids = {target["expected_characteristic_id"] for target in LOW_INCOME_TARGETS}
    target_rows = qc[qc["CHARACTERISTIC_ID"].isin(target_ids)].copy()

    if not target_rows.empty:
        target_rows_all.append(target_rows)
...
summary = pd.DataFrame(
    [
        ...
        {
            "metric": "low_income_targets_total",
            "value": len(LOW_INCOME_TARGETS),
        },
        {
            "metric": "low_income_targets_ready_for_cleaner",
            "value": ready_target_count,
        },
        {
            "metric": "recommended_next_step",
            "value": (
                "Generate clean_census_division_low_income_2021.py."
                if ready_target_count >= 1
                else "Review target summary before generating low-income cleaner."
            ),
        },
    ]
)
...
print("\n" + "=" * 72)
print("CENSUS DIVISION LOW-INCOME INSPECTION SUMMARY")
print("=" * 72)
...
print("\nLow-income candidate characteristics:")
