...
# ============================================================
# Clean Census Division Education Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division education feature table from the
#   2021 Census Profile at census-division level.
#
# SoVI variable cleaned here:
#
...
#
#   PCTNOHSDP90:
#       Original SoVI concept: percent persons with no high school diploma.
#       Local Canadian 2021 source:
...
#           C10_RATE_TOTAL
#
# Audit / sensitivity variables also included:
#
#   1996 -> pct_no_high_school_diploma_25_64
#   1999 -> pct_no_certificate_diploma_or_degree_15_plus
#   2015 -> pct_no_certificate_diploma_or_degree_25_64
#
# Validated by:
#   census_division_education_2021/inspect_census_division_education_2021.py
#
# Run from data/:
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_education_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_education_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_education_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_education_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_education_summary_2021.csv"
...
EDUCATION_TARGETS = [
    {
        ...
        "description": "Percent population with no high school diploma or equivalency certificate",
        "characteristic_id": "1993",
        "expected_parent_characteristic_id": "1992",
        "expected_parent_context": (
            "Total - Secondary (high) school diploma or equivalency certificate "
            "for the population aged 15 years and over in private households - 25% sample data"
        ),
        "expected_characteristic_name": "No high school diploma or equivalency certificate",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "no_high_school_diploma_15_plus_rate",
        "sovi_role": "main_no_high_school_diploma_proxy",
        "preferred": True,
        "methodological_choice": (
            "Original SoVI uses percent persons with no high school diploma. "
            "The selected Canadian 2021 proxy uses the Census Profile row "
            "'No high school diploma or equivalency certificate' under the "
            "population aged 15 years and over universe."
        ),
        "notes": (
            "Main SoVI-compatible education variable. Characteristic 1993 is selected "
            "over 1996 because the original SoVI variable is a general population "
            "education vulnerability variable rather than specifically a 25-to-64 "
            "working-age attainment variable."
        ),
    },
    {
        "original_code": "PCTNOHSDP90_ALT_25_64",
        "canonical_variable": "pct_no_high_school_diploma_25_64",
        "description": "Alternative percent population aged 25 to 64 with no high school diploma",
        "characteristic_id": "1996",
        "expected_parent_characteristic_id": "1995",
        "expected_parent_context": (
            "Total - Secondary (high) school diploma or equivalency certificate "
            "for the population aged 25 to 64 years in private households - 25% sample data"
        ),
        "expected_characteristic_name": "No high school diploma or equivalency certificate",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "no_high_school_diploma_25_64_rate",
        "sovi_role": "audit_alternative_no_high_school_diploma_proxy",
        "preferred": False,
        "methodological_choice": (
            "Kept as an audit/sensitivity alternative. This version restricts the "
            "universe to population aged 25 to 64 years, which may better represent "
            "completed adult educational attainment but is not the selected default "
            "SoVI proxy."
        ),
        "notes": (
            "Audit alternative. Useful for sensitivity analysis if the methodology later "
            "chooses a core-working-age education attainment universe."
        ),
    },
    {
        "original_code": "PCTNOHSDP90_ALT_NO_CERT_15_PLUS",
        "canonical_variable": "pct_no_certificate_diploma_or_degree_15_plus",
        "description": "Alternative percent population aged 15+ with no certificate, diploma or degree",
        "characteristic_id": "1999",
        "expected_parent_characteristic_id": "1998",
        "expected_parent_context": (
            "Total - Highest certificate, diploma or degree for the population aged "
            "15 years and over in private households - 25% sample data"
        ),
        "expected_characteristic_name": "No certificate, diploma or degree",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "no_certificate_diploma_or_degree_15_plus_rate",
        "sovi_role": "audit_alternative_no_certificate_proxy",
        "preferred": False,
        "methodological_choice": (
            "Kept as an audit/sensitivity alternative. This is a broader no-credential "
            "measure and is not as literal a match to the original no-high-school-diploma "
            "SoVI variable."
        ),
        "notes": (
            "Audit alternative. It measures no certificate, diploma or degree among "
            "population aged 15 years and over."
        ),
    },
    {
        "original_code": "PCTNOHSDP90_ALT_NO_CERT_25_64",
        "canonical_variable": "pct_no_certificate_diploma_or_degree_25_64",
        "description": "Alternative percent population aged 25 to 64 with no certificate, diploma or degree",
        "characteristic_id": "2015",
        "expected_parent_characteristic_id": "2014",
        "expected_parent_context": (
            "Total - Highest certificate, diploma or degree for the population aged "
            "25 to 64 years in private households - 25% sample data"
        ),
        "expected_characteristic_name": "No certificate, diploma or degree",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "no_certificate_diploma_or_degree_25_64_rate",
        "sovi_role": "audit_alternative_no_certificate_proxy",
        "preferred": False,
        "methodological_choice": (
            "Kept as an audit/sensitivity alternative. This is a broader no-credential "
            "measure restricted to ages 25 to 64, and is not the selected default SoVI proxy."
        ),
        "notes": (
            "Audit alternative. It may be useful if the methodology later chooses a "
            "core-working-age no-credential framing."
        ),
    },
]
...
target_characteristic_ids = sorted(
    {target["characteristic_id"] for target in EDUCATION_TARGETS}
)

print("\nCleaning Census Division Education Features 2021")
...
print("Rows:", len(base))
print(
    "Base names with possible mojibake:",
    int(base["census_division_name"].apply(looks_like_mojibake).sum()),
)
...
for chunk_idx, chunk in enumerate(
    ...
    for target in EDUCATION_TARGETS:
        ...
        for _, row in target_rows.iterrows():
            ...
            value_numeric = clean_numeric(pd.Series([value_raw])).iloc[0]
            ...
            long_rows.append(
                {
                    ...
                    "parent_characteristic_id": target["expected_parent_characteristic_id"],
                    "parent_context": target["expected_parent_context"],
                    ...
                    "methodological_choice": target["methodological_choice"],
...
if source_long.empty:
    raise ValueError(
        "No target education rows were extracted from the raw Census Profile file."
    )
...
if not not_ready.empty:
    raise ValueError(
        "One or more education variables failed validation:\n"
...
for target in EDUCATION_TARGETS:
    ...
    clean[f"{var}__source_parent_characteristic_id"] = target["expected_parent_characteristic_id"]
    clean[f"{var}__source_parent_context"] = target["expected_parent_context"]
    clean[f"{var}__source_value_column"] = target["value_column"]
    clean[f"{var}__unit"] = target["unit"]
    clean[f"{var}__sovi_role"] = target["sovi_role"]
    clean[f"{var}__preferred"] = target["preferred"]
    clean[f"{var}__methodological_choice"] = target["methodological_choice"]

clean["education_feature_count"] = len(EDUCATION_TARGETS)
clean["education_preferred_feature_count"] = sum(
    1 for target in EDUCATION_TARGETS if target["preferred"]
)
clean["education_audit_feature_count"] = sum(
    1 for target in EDUCATION_TARGETS if not target["preferred"]
)
clean["education_missing_count"] = clean[
    [target["canonical_variable"] for target in EDUCATION_TARGETS]
].isna().sum(axis=1)

clean["education_complete"] = clean["education_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, education section"
...
summary_rows = [
    ...
    {
        "metric": "preferred_variable",
        "value": "pct_no_high_school_diploma",
    },
    {
        "metric": "audit_variables",
        "value": ", ".join(
            target["canonical_variable"]
            for target in EDUCATION_TARGETS
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in EDUCATION_TARGETS),
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["education_complete"].all()),
    },
    {
...
for target in EDUCATION_TARGETS:
    var = target["canonical_variable"]
    series_summary = summarize_series(clean[var])

    for metric_name, value in series_summary.items():
        summary_rows.append(
            {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION EDUCATION FEATURES 2021")
print("=" * 72)
...
print(
    "Base names with possible mojibake:",
    int(clean["census_division_name_base_had_mojibake"].sum()),
)
print(
    "Profile names with possible mojibake:",
    int(clean["profile_census_division_name_had_mojibake"].sum()),
)
...
for target in EDUCATION_TARGETS:
    var = target["canonical_variable"]
    preferred_label = "preferred" if target["preferred"] else "audit"
    print(
        f"- {var} ({preferred_label}): "
...
for target in EDUCATION_TARGETS:
    print(f"- {target['canonical_variable']}: {target['methodological_choice']}")
