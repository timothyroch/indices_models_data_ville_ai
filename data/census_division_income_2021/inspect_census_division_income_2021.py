...
# ============================================================
# Inspect Census Division Income Features 2021
# ============================================================
#
# Purpose:
#   Inspect the 2021 Census Profile at census-division level for income
...
#   PCTHH7589  -> pct_high_income_households / high-income household proxy
#
# This script does NOT create the final clean income table.
...
OUTPUT_FILE_INVENTORY = OUTPUT_DIR / "income_file_inventory_2021.csv"
OUTPUT_CD_INVENTORY = OUTPUT_DIR / "income_quebec_cd_inventory_2021.csv"
OUTPUT_BASE_JOIN = OUTPUT_DIR / "income_quebec_cd_base_join_check_2021.csv"
OUTPUT_CANDIDATE_CHARACTERISTICS = OUTPUT_DIR / "income_candidate_characteristics_2021.csv"
OUTPUT_TARGET_SUMMARY = OUTPUT_DIR / "income_target_characteristic_summary_2021.csv"
OUTPUT_TARGET_VALUES_LONG = OUTPUT_DIR / "income_target_values_long_2021.csv"
OUTPUT_TARGET_VALUES_WIDE = OUTPUT_DIR / "income_target_values_wide_2021.csv"
OUTPUT_SYMBOL_COUNTS = OUTPUT_DIR / "income_symbol_counts_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "income_inspection_summary_2021.csv"
...
#
# This is intentionally an inspection range, not a final cleaner range.
INCOME_CONTEXT_MIN_ID = 100
INCOME_CONTEXT_MAX_ID = 370
...
# These targets are inspection targets.
# The selected high-income threshold is intentionally not final.
# We inspect several plausible high-income household thresholds and let the
# output tell us what exists in the CD profile.
INCOME_TARGETS = [
    {
        "original_code": "PERCAP89",
        "canonical_variable": "income_measure_default",
        "candidate_output_alias": "income_measure_default",
        "description": "Income proxy for original SoVI per capita income",
        "target_family": "person_income_statistic",
        ...
        "notes": (
            "Original SoVI uses per capita income. Census Profile may not provide a direct per-capita-income row. "
            "This inspection searches for person-level income statistics such as average or median total/after-tax income."
        ),
    },
    {
        "original_code": "PCTHH7589",
        "canonical_variable": "pct_households_income_100k_plus",
        "candidate_output_alias": "pct_high_income_households",
        "description": "Percent high-income households, modern Canadian proxy",
        "target_family": "household_income_threshold",
        "preferred_name_contains_any": [
            "$100,000 and over",
            "$100,000 to $124,999",
            "$125,000 to $149,999",
        ...
        "notes": (
            "Original SoVI used percent households earning more than $75,000 in 1989. "
            "For 2021 Canada, the cleaner will need a documented modern threshold, likely $100,000+ or another reviewed household-income cut."
        ),
    },
]
...
def is_income_candidate(row: pd.Series) -> bool:
    ...
    in_context_range = (
        char_id is not None
        and INCOME_CONTEXT_MIN_ID <= char_id <= INCOME_CONTEXT_MAX_ID
    )

    keyword_match = any(keyword.lower() in name for keyword in INCOME_KEYWORDS_ANY)
    excluded = any(keyword.lower() in name for keyword in INCOME_KEYWORDS_EXCLUDE)
...
print("\nCensus Profile CD income inspection")
print("Raw file:", RAW_CSV.relative_to(DATA_DIR))
print("Raw encoding selected:", raw_encoding)
print("Income context ID range:", f"{INCOME_CONTEXT_MIN_ID}–{INCOME_CONTEXT_MAX_ID}")
...
for chunk_idx, chunk in enumerate(
    ...
    candidate_mask = qc.apply(is_income_candidate, axis=1)
    candidates = qc.loc[candidate_mask].copy()

    if not candidates.empty:
        candidate_rows.append(candidates)

        # Target-specific candidate rows.
...
if not target_values_long.empty:
    # This wide file is only exploratory, because multiple candidate characteristics
    # may exist per target. It includes characteristic IDs in the column names.
...
summary = pd.DataFrame(
    [
        ...
        },
        {
            ...
            "value": (
                "Review income_target_characteristic_summary_2021.csv before generating cleaner. "
                "Income requires an explicit methodological choice for the default income proxy and high-income threshold."
            ),
        },
    ]
)
...
print("\n" + "=" * 72)
print("CENSUS DIVISION INCOME INSPECTION SUMMARY")
print("=" * 72)
...
print("\nIncome candidate characteristics:")
