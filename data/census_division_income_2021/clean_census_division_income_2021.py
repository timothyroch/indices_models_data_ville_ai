...
# ============================================================
# Clean Census Division Income Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division income feature table from the
...
#
#   PERCAP89:
#       Original SoVI concept: per capita income.
...
#
#       Rationale:
#           The 2021 Census Profile does not provide a direct per-capita-income
#           row in the same simple way as the original SoVI variable. Among the
#           available person-level candidates, average total income among
#           recipients is the closest broad income-capacity proxy.
#
#   PCTHH7589:
#       Original SoVI concept: percent households earning more than $75,000.
#       Local 2021 proxy:
...
#
#       Rationale:
#           The original $75,000 threshold is from 1989 U.S. dollars. For 2021
#           Canadian Census Profile data, $100,000+ is used as a documented
#           modern high-income household threshold proxy.
#
# Validated by:
#   census_division_income_2021/inspect_census_division_income_2021.py
#
# Run from data/:
...
SECTION_DIR = DATA_DIR / "census_division_income_2021"
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_income_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_income_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_income_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_income_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_income_summary_2021.csv"
...
INCOME_TARGETS = [
    {
        ...
        "canonical_variable": "income_measure_default",
        "description": "Income proxy for original SoVI per capita income",
        "characteristic_id": "128",
        "expected_characteristic_name": "Average total income in 2020 among recipients ($)",
        "value_column": "C1_COUNT_TOTAL",
        "symbol_column": "SYMBOL",
        "unit": "dollars",
        "source_value_type": "average_total_income_recipients",
        "sovi_role": "local_income_proxy",
        "methodological_choice": (
            "Original SoVI uses per capita income. This local Canadian 2021 proxy uses "
            "average total income in 2020 among recipients because it is the closest "
            "broad person-level income-capacity candidate available in the Census Profile."
        ),
        "notes": (
            "Alternative candidates inspected included median total income, median after-tax "
            "income, and average after-tax income among recipients."
        ),
    },
    {
        "original_code": "PCTHH7589",
        "canonical_variable": "pct_high_income_households",
        "description": "Percent high-income households using a modern 2021 Canadian threshold",
        "characteristic_id": "276",
        "expected_characteristic_name": "$100,000 and over",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "household_income_100k_plus_rate",
        "sovi_role": "modern_high_income_household_proxy",
        "methodological_choice": (
            "Original SoVI uses percent households earning more than $75,000 in 1989. "
            "This local Canadian 2021 proxy uses the Census Profile household-income "
            "$100,000 and over category as a documented modern threshold."
        ),
        "notes": (
            "Alternative candidates inspected included $100,000 to $124,999, "
...
target_characteristic_ids = sorted({target["characteristic_id"] for target in INCOME_TARGETS})

print("\nCleaning Census Division Income Features 2021")
...
if source_long.empty:
    raise ValueError("No target income rows were extracted from the raw Census Profile file.")
...
if not not_ready.empty:
    raise ValueError(
        "One or more income variables failed validation:\n"
...
# Helpful explicit aliases for readability.
clean["average_total_income_2020_recipients"] = clean["income_measure_default"]
clean["pct_households_income_100k_plus"] = clean["pct_high_income_households"]

clean["income_feature_count"] = len(INCOME_TARGETS)
clean["income_missing_count"] = clean[
    [target["canonical_variable"] for target in INCOME_TARGETS]
].isna().sum(axis=1)

clean["income_complete"] = clean["income_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, income section"
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in INCOME_TARGETS),
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["income_complete"].all()),
    },
    {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION INCOME FEATURES 2021")
print("=" * 72)
