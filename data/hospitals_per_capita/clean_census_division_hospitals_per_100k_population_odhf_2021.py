...
# ============================================================
# Clean Census Division Hospitals per 100k Population from ODHF
# ============================================================
#
# Purpose:
#   Compute a census-division-level hospitals-per-100k-population feature
#   using:
#
#   Numerator:
#       ODHF hospital facility record counts by census division
#
#   Denominator:
...
#
# Run from data/:
#   python hospitals_per_capita/clean_census_division_hospitals_per_100k_population_odhf_2021.py
#
# ============================================================
...
SOURCE_HOSPITALS = "Statistics Canada Open Database of Healthcare Facilities (ODHF), version 1.1"
SOURCE_POPULATION = (
...
FEATURE_DESCRIPTION = (
    "ODHF hospital facility records per 100,000 population by 2021 Quebec census division. "
    "The numerator is hospital_count_odhf from ODHF records classified as Hospitals. "
    "The denominator is population_total_2021 from Statistics Canada Table 98-10-0007-01. "
    "This is a facility-record proxy, not an official count of unique hospital systems."
)
...
if not HOSPITAL_COUNTS_PATH.exists():
    raise FileNotFoundError(f"Hospital counts file not found:\n{HOSPITAL_COUNTS_PATH}")
...
hospital_counts = pd.read_csv(HOSPITAL_COUNTS_PATH, dtype=str)
cd_population = gpd.read_file(CD_POPULATION_GEO_PATH)

print("\nLoaded hospital counts")
print("Rows:", len(hospital_counts))
print("Columns:", list(hospital_counts.columns))
...
require_columns(hospital_counts, required_hospital_cols, "hospital counts file")
require_columns(cd_population, required_population_cols, "CD population spatial frame")
...
hospital_counts = hospital_counts.copy()
...
for col in [
    ...
    hospital_counts[col] = clean_text(hospital_counts[col])
    cd_population[col] = clean_text(cd_population[col])

numeric_hospital_cols = [
    "hospital_count_odhf",
    "hospital_count_automatic_csd_uid",
    "hospital_count_manual_repair",
]

for col in numeric_hospital_cols:
    hospital_counts[col] = pd.to_numeric(hospital_counts[col], errors="coerce")
...
if len(hospital_counts) != 98:
    raise ValueError(f"Expected 98 hospital-count rows, got {len(hospital_counts)}.")

if len(cd_population) != 98:
    raise ValueError(f"Expected 98 population rows, got {len(cd_population)}.")

if hospital_counts["census_division_dguid"].duplicated().any():
    duplicated = hospital_counts[
        hospital_counts["census_division_dguid"].duplicated(keep=False)
    ]
    raise ValueError(
        "Duplicated census_division_dguid values in hospital counts:\n"
        + duplicated[
            [
                "census_division_code",
                "census_division_dguid",
...
# -----------------------------
# Join hospital counts to population frame
# -----------------------------

joined = hospital_counts.merge(
    population_join,
...
if not unmatched.empty:
    raise ValueError(
        "Some hospital-count rows did not match the population frame:\n"
        + unmatched[
            [
                "census_division_code",
                "census_division_dguid",
...
if (
    joined["hospital_count_automatic_csd_uid"]
    + joined["hospital_count_manual_repair"]
    != joined["hospital_count_odhf"]
).any():
    bad = joined[
        (
            joined["hospital_count_automatic_csd_uid"]
    ...
    raise ValueError(
        "Automatic + manual hospital counts do not equal total hospital count:\n"
        + bad[
            [
                "census_division_code",
                "census_division_name",
...
joined["hospitals_per_100k_population_odhf"] = (
    joined["hospital_count_odhf"]
    / joined["population_total_2021"]
    * 100000
)

joined["hospitals_per_10k_population_odhf"] = (
    joined["hospital_count_odhf"]
    / joined["population_total_2021"]
    * 10000
)

joined["hospital_count_per_km2_odhf"] = (
    joined["hospital_count_odhf"]
    / joined["land_area_km2"]
)

joined["has_hospital_odhf"] = joined["hospital_count_odhf"] > 0
...
joined["source_hospitals"] = SOURCE_HOSPITALS
...
if gdf["hospitals_per_100k_population_odhf"].isna().any():
    bad = gdf[gdf["hospitals_per_100k_population_odhf"].isna()]
    raise ValueError(
        "Missing hospitals_per_100k_population_odhf values:\n"
        + bad[
            [
                "census_division_code",
                "census_division_name",
...
if (gdf["hospitals_per_100k_population_odhf"] < 0).any():
    bad = gdf[gdf["hospitals_per_100k_population_odhf"] < 0]
    raise ValueError(
        "Negative hospitals_per_100k_population_odhf values:\n"
        + bad[
            [
                "census_division_code",
                "census_division_name",
...
gdf.to_file(OUTPUT_GPKG, layer="hospitals_per_100k_population_odhf_2021", driver="GPKG")
...
print("\nFinal hospitals-per-100k table")
print("Rows:", len(gdf))
print("CRS:", gdf.crs)

print("\nCount totals:")
print("Total hospital_count_odhf:", int(gdf["hospital_count_odhf"].sum()))
print("Total automatic assignments:", int(gdf["hospital_count_automatic_csd_uid"].sum()))
print("Total manual repairs:", int(gdf["hospital_count_manual_repair"].sum()))
print("Census divisions with at least one hospital:", int(gdf["has_hospital_odhf"].sum()))
print("Census divisions with zero hospitals:", int((~gdf["has_hospital_odhf"]).sum()))

print("\nRate summary:")
print(gdf["hospitals_per_100k_population_odhf"].describe().to_string())

print("\nTop 20 census divisions by hospitals per 100k population:")
print(
    gdf[
        [
            "census_division_code",
    ...
    .sort_values("hospitals_per_100k_population_odhf", ascending=False)
    .head(20)
    .to_string(index=False)
)

print("\nTop 20 census divisions by hospital count:")
print(
    gdf[
        [
            "census_division_code",
