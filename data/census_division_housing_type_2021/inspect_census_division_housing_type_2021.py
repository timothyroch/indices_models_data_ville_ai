...
# ============================================================
# Inspect Census Division Housing Type Features 2021
# ============================================================
#
# Purpose:
#   Inspect the 2021 Census Profile at census-division level for housing-type
...
#   Movable dwelling
#
# This script does NOT create the final clean housing-type table.
...
SECTION_DIR = DATA_DIR / "census_division_housing_type_2021"
...
OUTPUT_FILE_INVENTORY = OUTPUT_DIR / "housing_type_file_inventory_2021.csv"
OUTPUT_CD_INVENTORY = OUTPUT_DIR / "housing_type_quebec_cd_inventory_2021.csv"
OUTPUT_BASE_JOIN = OUTPUT_DIR / "housing_type_quebec_cd_base_join_check_2021.csv"
OUTPUT_CANDIDATE_CHARACTERISTICS = OUTPUT_DIR / "housing_type_candidate_characteristics_2021.csv"
OUTPUT_TARGET_SUMMARY = OUTPUT_DIR / "housing_type_target_characteristic_summary_2021.csv"
OUTPUT_TARGET_VALUES_LONG = OUTPUT_DIR / "housing_type_target_values_long_2021.csv"
OUTPUT_TARGET_VALUES_WIDE = OUTPUT_DIR / "housing_type_target_values_wide_2021.csv"
OUTPUT_SYMBOL_COUNTS = OUTPUT_DIR / "housing_type_symbol_counts_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "housing_type_inspection_summary_2021.csv"
...
# The housing-type section is usually near the early dwelling/household rows.
# Earlier CT-level work suggested movable dwelling is around CHARACTERISTIC_ID 49.
HOUSING_TYPE_CONTEXT_IDS = {
    "35",
    "36",
    "37",
    "38",
...
HOUSING_TYPE_TARGETS = [
    {
        "original_code": "PCTMOBL90",
        "canonical_variable": "pct_mobile_homes",
        "candidate_output_alias": "pct_movable_dwelling",
        "description": "Percent mobile homes / movable dwellings",
        "target_family": "housing_type",
        "expected_characteristic_id": "49",
        "expected_characteristic_name_contains": "movable dwelling",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "sovi_role": "movable_dwelling_proxy_for_mobile_homes",
        "preferred": True,
        "notes": (
            "Original SoVI uses percent mobile homes. The Canadian Census Profile "
            "category 'Movable dwelling' is expected to be the closest available proxy."
        ),
    },
]
...
def is_housing_type_candidate(row: pd.Series) -> bool:
    char_id = str(row.get("CHARACTERISTIC_ID", "")).strip()
    name = normalize_text(row.get("CHARACTERISTIC_NAME", ""))

    id_match = char_id in HOUSING_TYPE_CONTEXT_IDS
    keyword_match = any(keyword.lower() in name for keyword in HOUSING_TYPE_KEYWORDS_ANY)
...
print("\nCensus Profile CD housing-type inspection")
print("Raw file:", RAW_CSV.relative_to(DATA_DIR))
print("Raw encoding selected:", raw_encoding)
print("Target characteristic IDs:", sorted({target["expected_characteristic_id"] for target in HOUSING_TYPE_TARGETS}))
...
for chunk_idx, chunk in enumerate(
    ...
    candidate_mask = qc.apply(is_housing_type_candidate, axis=1)
    candidates = qc.loc[candidate_mask].copy()

    if not candidates.empty:
        candidate_rows.append(candidates)

    target_ids = {target["expected_characteristic_id"] for target in HOUSING_TYPE_TARGETS}
...
for target in HOUSING_TYPE_TARGETS:
...
summary = pd.DataFrame(
    [
        ...
        {
            "metric": "housing_type_targets_total",
            "value": len(HOUSING_TYPE_TARGETS),
        },
        {
            "metric": "housing_type_targets_ready_for_cleaner",
            "value": ready_target_count,
        },
        {
            "metric": "recommended_next_step",
            "value": (
                "Generate clean_census_division_housing_type_2021.py."
                if ready_target_count == len(HOUSING_TYPE_TARGETS)
                else "Review target summary before generating housing-type cleaner."
            ),
        },
    ]
)
...
print("\n" + "=" * 72)
print("CENSUS DIVISION HOUSING TYPE INSPECTION SUMMARY")
print("=" * 72)
...
print("\nHousing-type candidate characteristics:")
