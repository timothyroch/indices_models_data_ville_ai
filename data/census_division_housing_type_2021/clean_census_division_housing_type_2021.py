...
# ============================================================
# Clean Census Division Housing Type Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division housing-type feature table from the
...
#
#   PCTMOBL90:
#       Original SoVI concept: percent mobile homes.
...
#
#       Rationale:
#           The Canadian Census Profile category "Movable dwelling" is the
#           closest available equivalent to the original SoVI mobile-homes
#           variable.
#
# Validated by:
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_housing_type_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_housing_type_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_housing_type_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_housing_type_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_housing_type_summary_2021.csv"
...
HOUSING_TYPE_TARGETS = [
    {
        ...
        "characteristic_id": "49",
        "expected_characteristic_name": "Movable dwelling",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "movable_dwelling_rate",
        "sovi_role": "movable_dwelling_proxy_for_mobile_homes",
        "methodological_choice": (
            "Original SoVI uses percent mobile homes. The 2021 Canadian Census "
            "Profile category 'Movable dwelling' is used as the closest available "
            "proxy."
        ),
        "notes": (
            "Values are published Census Profile percentage rates for the movable "
...
target_characteristic_ids = sorted(
    {target["characteristic_id"] for target in HOUSING_TYPE_TARGETS}
)

print("\nCleaning Census Division Housing Type Features 2021")
...
if source_long.empty:
    raise ValueError(
        "No target housing-type rows were extracted from the raw Census Profile file."
    )
...
if not not_ready.empty:
    raise ValueError(
        "One or more housing-type variables failed validation:\n"
...
clean["pct_movable_dwelling"] = clean["pct_mobile_homes"]

clean["housing_type_feature_count"] = len(HOUSING_TYPE_TARGETS)
clean["housing_type_missing_count"] = clean[
    [target["canonical_variable"] for target in HOUSING_TYPE_TARGETS]
].isna().sum(axis=1)

clean["housing_type_complete"] = clean["housing_type_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, housing type section"
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in HOUSING_TYPE_TARGETS),
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["housing_type_complete"].all()),
    },
    {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION HOUSING TYPE FEATURES 2021")
print("=" * 72)
