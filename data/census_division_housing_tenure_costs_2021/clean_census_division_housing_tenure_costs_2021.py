...
# ============================================================
# Clean Census Division Housing Tenure/Costs Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division housing tenure/costs feature table
#   from the 2021 Census Profile at census-division level.
#
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_housing_tenure_costs_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_housing_tenure_costs_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_housing_tenure_costs_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_housing_tenure_costs_variable_metadata_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_housing_tenure_costs_summary_2021.csv"
...
HOUSING_TARGETS = [
    {
        ...
        "characteristic_id": "1416",
        "expected_characteristic_name": "Renter",
        "value_column": "C10_RATE_TOTAL",
        "symbol_column": "SYMBOL.3",
        "unit": "percent",
        "source_value_type": "renter_tenure_rate",
        "sovi_role": "renter_tenure_proxy",
        "methodological_choice": (
            "Original SoVI uses percent renter-occupied housing units. "
            "The 2021 Census Profile row 'Renter' is used as the broad "
            "Canadian renter-tenure proxy. Shelter-cost-to-income denominator "
            "rows containing 'tenant' are intentionally ignored."
        ),
        "notes": (
            "Inspection also matched rows 1465, 1479, and 1490 due to the word "
            "'tenant', but those are not the correct renter-tenure percentage. "
            "Row 1416 is the selected source."
        ),
    },
    {
        ...
        "characteristic_id": "1494",
        "expected_characteristic_name": "Median monthly shelter costs for rented dwellings ($)",
        "value_column": "C1_COUNT_TOTAL",
        "symbol_column": "SYMBOL",
        "unit": "dollars",
        "source_value_type": "median_monthly_shelter_costs_rented",
        "sovi_role": "renter_housing_cost_proxy",
        "methodological_choice": (
            "Original SoVI uses median rent. The Canadian Census Profile proxy "
            "uses median monthly shelter costs for rented dwellings."
        ),
        "notes": (
            "This is a Canadian Census Profile shelter-cost proxy for median rent."
        ),
    },
    {
        ...
        "characteristic_id": "1488",
        "expected_characteristic_name": "Median value of dwellings ($)",
        "value_column": "C1_COUNT_TOTAL",
        "symbol_column": "SYMBOL",
        "unit": "dollars",
        "source_value_type": "median_value_of_dwellings",
        "sovi_role": "owner_housing_value_proxy",
        "methodological_choice": (
            "Original SoVI uses median dollar value of owner-occupied housing. "
            "The Canadian Census Profile proxy uses median value of dwellings."
        ),
        "notes": (
            "This is a dwelling-value proxy for the original owner-occupied housing value variable."
        ),
    },
]
...
target_characteristic_ids = sorted({target["characteristic_id"] for target in HOUSING_TARGETS})

print("\nCleaning Census Division Housing Tenure/Costs Features 2021")
...
if source_long.empty:
    raise ValueError(
        "No target housing tenure/costs rows were extracted from the raw Census Profile file."
    )
...
if not not_ready.empty:
    raise ValueError(
        "One or more housing tenure/costs variables failed validation:\n"
...
clean["pct_renter_households"] = clean["pct_renter_occupied"]
clean["median_monthly_shelter_costs_rented"] = clean["median_rent"]
clean["median_value_of_dwellings"] = clean["median_owner_occupied_housing_value"]

clean["housing_tenure_costs_feature_count"] = len(HOUSING_TARGETS)
clean["housing_tenure_costs_missing_count"] = clean[
    [target["canonical_variable"] for target in HOUSING_TARGETS]
].isna().sum(axis=1)

clean["housing_tenure_costs_complete"] = clean["housing_tenure_costs_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, housing tenure/costs section"
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in HOUSING_TARGETS),
    },
    {
        "metric": "all_variables_complete",
        "value": bool(clean["housing_tenure_costs_complete"].all()),
    },
    {
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION HOUSING TENURE/COSTS FEATURES 2021")
print("=" * 72)
