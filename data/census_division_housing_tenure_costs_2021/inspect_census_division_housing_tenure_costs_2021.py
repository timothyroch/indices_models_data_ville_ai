...
# ============================================================
# Inspect Census Division Housing Tenure/Costs Features 2021
# ============================================================
#
# Purpose:
#   Inspect the 2021 Census Profile at census-division level for housing
#   tenure and housing-cost variables needed by the SoVI-like
...
#
# This script does NOT create the final clean housing table.
...
SECTION_DIR = DATA_DIR / "census_division_housing_tenure_costs_2021"
...
OUTPUT_FILE_INVENTORY = OUTPUT_DIR / "housing_tenure_costs_file_inventory_2021.csv"
OUTPUT_CD_INVENTORY = OUTPUT_DIR / "housing_tenure_costs_quebec_cd_inventory_2021.csv"
OUTPUT_BASE_JOIN = OUTPUT_DIR / "housing_tenure_costs_quebec_cd_base_join_check_2021.csv"
OUTPUT_CANDIDATE_CHARACTERISTICS = OUTPUT_DIR / "housing_tenure_costs_candidate_characteristics_2021.csv"
OUTPUT_TARGET_SUMMARY = OUTPUT_DIR / "housing_tenure_costs_target_characteristic_summary_2021.csv"
OUTPUT_TARGET_VALUES_LONG = OUTPUT_DIR / "housing_tenure_costs_target_values_long_2021.csv"
OUTPUT_TARGET_VALUES_WIDE = OUTPUT_DIR / "housing_tenure_costs_target_values_wide_2021.csv"
OUTPUT_SYMBOL_COUNTS = OUTPUT_DIR / "housing_tenure_costs_symbol_counts_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "housing_tenure_costs_inspection_summary_2021.csv"
...
# Broad inspection range. Housing tenure/cost rows can appear in the housing
# section with IDs well after the income section.
HOUSING_CONTEXT_MIN_ID = 1400
HOUSING_CONTEXT_MAX_ID = 1550

HOUSING_KEYWORDS_ANY = [
    "owner",
    "renter",
    "tenant",
    "rented",
...
# These are inspection targets. If exact IDs differ, the target summary will
# show missing expected characteristics, while the candidate file will still
# reveal the relevant housing block.
HOUSING_TARGETS = [
    {
        "original_code": "PCTRENTER90",
        "canonical_variable": "pct_renter_occupied",
        "candidate_output_alias": "pct_renter_households",
        "description": "Percent renter-occupied households / renter tenure proxy",
        "target_family": "housing_tenure",
        "expected_characteristic_id": None,
        ...
        "unit": "percent",
        "sovi_role": "renter_tenure_proxy",
        "notes": (
            "Original SoVI uses percent renter-occupied housing units. "
            "The cleaner should use the broad renter/tenant tenure row, not shelter-cost rows."
        ),
    },
    {
        "original_code": "MEDRENT90",
        "canonical_variable": "median_rent",
        "candidate_output_alias": "median_monthly_shelter_costs_rented",
        "description": "Median rent / median monthly shelter costs for rented dwellings",
        "target_family": "housing_cost_renter",
        "expected_characteristic_id": None,
        "preferred_name_contains_any": [
        ...
        "unit": "dollars",
        "sovi_role": "renter_housing_cost_proxy",
        "notes": (
            "Original SoVI uses median rent. The Canadian Census Profile proxy is expected "
            "to be median monthly shelter costs for rented dwellings."
        ),
    },
    {
        "original_code": "MVALOO90",
        "canonical_variable": "median_owner_occupied_housing_value",
        "candidate_output_alias": "median_value_owner_occupied_dwelling",
        "description": "Median value of owner-occupied housing",
        "target_family": "housing_value_owner",
        "expected_characteristic_id": None,
        "preferred_name_contains_any": [
            "median value of dwellings",
        ...
        "symbol_column": "SYMBOL",
        "unit": "dollars",
        "sovi_role": "owner_housing_value_proxy",
        "notes": (
            "Original SoVI uses median dollar value of owner-occupied housing. "
            "The Canadian proxy is expected to be median value of dwellings among owner households."
        ),
    },
]
...
def is_housing_candidate(row: pd.Series) -> bool:
    ...
    in_context_range = (
        char_id is not None
        and HOUSING_CONTEXT_MIN_ID <= char_id <= HOUSING_CONTEXT_MAX_ID
    )

    keyword_match = any(keyword.lower() in name for keyword in HOUSING_KEYWORDS_ANY)

    return in_context_range or keyword_match


def target_matches_row(target: dict, row: pd.Series) -> tuple[bool, str]:
    name = normalize_text(row.get("CHARACTERISTIC_NAME", ""))

    expected_id = target.get("expected_characteristic_id")
    char_id = str(row.get("CHARACTERISTIC_ID", "")).strip()

    if expected_id is not None and str(expected_id) == char_id:
        return True, f"expected_characteristic_id:{expected_id}"
...
print("\nCensus Profile CD housing tenure/costs inspection")
print("Raw file:", RAW_CSV.relative_to(DATA_DIR))
print("Raw encoding selected:", raw_encoding)
print("Housing context ID range:", f"{HOUSING_CONTEXT_MIN_ID}–{HOUSING_CONTEXT_MAX_ID}")
...
for chunk_idx, chunk in enumerate(
    ...
    candidate_mask = qc.apply(is_housing_candidate, axis=1)
...
if not target_values_long.empty:
    # Exploratory wide output. Multiple candidate characteristics may exist
    # for each target, so the characteristic ID is included in the column name.
...
summary = pd.DataFrame(
    [
        ...
        },
        {
            ...
            "value": (
                "Review housing_tenure_costs_target_characteristic_summary_2021.csv before generating cleaner. "
                "Housing tenure/costs may require explicit selection among several plausible candidate rows."
            ),
        },
    ]
)
...
print("\n" + "=" * 72)
print("CENSUS DIVISION HOUSING TENURE/COSTS INSPECTION SUMMARY")
print("=" * 72)
...
print("\nHousing candidate characteristics:")
