...
# ============================================================
# Clean Census Division Household / Family Features 2021
# ============================================================
#
# Purpose:
#   Build a clean Québec census-division household/family feature table from
#   the 2021 Census Profile at census-division level.
#
# SoVI variables addressed here:
#
#   AVGPERHH  -> avg_people_per_household
#   PCTF_HH90 -> unresolved in this source
#
# Methodological choices:
#
#   AVGPERHH:
#       Original SoVI concept: average number of persons per household.
...
#
#   PCTF_HH90:
#       Original SoVI concept: percent female-headed households.
#       Current status:
#           Not cleaned as a numeric feature here because the targeted
#           inspection did not find a defensible numerator in the 2021 Census
#           Profile CD file.
#
#       Inspection result:
#           The pipeline found possible private-household denominator rows but
#           no complete female-parent / women household-maintainer numerator
#           suitable for a clean derived proxy.
#
#       Output:
#           The cleaner records unresolved metadata columns for this variable,
#           but does not create a numeric pct_female_headed_households feature.
#
# Validated by:
...
OUTPUT_CSV = OUTPUT_DIR / "clean_census_division_household_family_2021.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "clean_census_division_household_family_2021.parquet"
OUTPUT_SOURCE_LONG = OUTPUT_DIR / "clean_census_division_household_family_source_long_2021.csv"
OUTPUT_VARIABLE_METADATA = OUTPUT_DIR / "clean_census_division_household_family_variable_metadata_2021.csv"
OUTPUT_UNRESOLVED_VARIABLES = OUTPUT_DIR / "clean_census_division_household_family_unresolved_variables_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "clean_census_division_household_family_summary_2021.csv"
...
HOUSEHOLD_FAMILY_TARGETS = [
    {
        ...
        "characteristic_id": "57",
        "expected_characteristic_name": "Average household size",
        "value_column": "C1_COUNT_TOTAL",
        "symbol_column": "SYMBOL",
        "unit": "persons_per_household",
        "source_value_type": "average_household_size",
        "sovi_role": "average_household_size_proxy",
        "methodological_choice": (
            "Original SoVI uses average number of persons per household. "
            "The 2021 Canadian Census Profile row 'Average household size' "
            "is used directly."
        ),
        "notes": (
            "Validated with complete coverage for all 98 Québec census divisions."
        ),
    },
]
...
UNRESOLVED_VARIABLES = [
    {
        ...
        "description": "Percent female-headed households",
        "status": "unresolved_not_cleaned",
        "reason": (
            "The broad household/family inspection and targeted female-headed-household "
            "proxy inspection did not find a defensible numerator in the CD-level 2021 "
            "Census Profile file. The inspections found possible total private-household "
            "denominators, but no complete female-parent / women household-maintainer "
            "numerator suitable for a clean proxy."
        ),
        "recommended_action": (
            "Keep this variable out of the numeric clean table for now. Search an "
            "additional Census family, household-maintainer, custom table, or other "
            "source if this SoVI variable must be included later."
        ),
        "included_as_numeric_feature": False,
    },
]
...
target_characteristic_ids = sorted(
    {target["characteristic_id"] for target in HOUSEHOLD_FAMILY_TARGETS}
)

print("\nCleaning Census Division Household / Family Features 2021")
...
print("Unresolved variables:", ", ".join(v["canonical_variable"] for v in UNRESOLVED_VARIABLES))
...
if source_long.empty:
    raise ValueError(
        "No target household/family rows were extracted from the raw Census Profile file."
    )
...
if not not_ready.empty:
    raise ValueError(
        "One or more household/family variables failed validation:\n"
...
# Helpful explicit alias for readability.
clean["avg_household_size"] = clean["avg_people_per_household"]

# Explicit unresolved markers. These are metadata columns, not numeric features.
clean["pct_female_headed_households__status"] = "unresolved_not_cleaned"
clean["pct_female_headed_households__included_as_numeric_feature"] = False
clean["pct_female_headed_households__reason"] = UNRESOLVED_VARIABLES[0]["reason"]

clean["household_family_feature_count"] = len(HOUSEHOLD_FAMILY_TARGETS)
clean["household_family_unresolved_feature_count"] = len(UNRESOLVED_VARIABLES)
clean["household_family_missing_count"] = clean[
    [target["canonical_variable"] for target in HOUSEHOLD_FAMILY_TARGETS]
].isna().sum(axis=1)

clean["household_family_complete"] = clean["household_family_missing_count"].eq(0)

clean["source"] = str(RAW_CSV.relative_to(DATA_DIR))
clean["source_section"] = "2021 Census Profile, Census divisions, household/family section"
...
for unresolved in UNRESOLVED_VARIABLES:
    metadata_rows.append(
        {
            "original_code": unresolved["original_code"],
            "canonical_variable": unresolved["canonical_variable"],
            "description": unresolved["description"],
            "status": unresolved["status"],
            "characteristic_id": "",
            "expected_characteristic_name": "",
            "value_column": "",
            "symbol_column": "",
            "unit": "",
            "source_value_type": "",
            "sovi_role": "",
            "methodological_choice": unresolved["reason"],
            "source_file": str(RAW_CSV.relative_to(DATA_DIR)),
            "n_rows": len(clean),
            "non_missing": "",
            "missing": "",
            ...
            "mean": "",
            "median": "",
            "notes": unresolved["recommended_action"],
        }
    )

metadata = pd.DataFrame(metadata_rows)

unresolved_df = pd.DataFrame(UNRESOLVED_VARIABLES)
...
summary_rows = [
    ...
    },
    {
        ...
        "value": ", ".join(target["canonical_variable"] for target in HOUSEHOLD_FAMILY_TARGETS),
    },
    {
        "metric": "variables_unresolved",
        "value": ", ".join(item["canonical_variable"] for item in UNRESOLVED_VARIABLES),
    },
    {
        "metric": "all_cleaned_variables_complete",
        "value": bool(clean["household_family_complete"].all()),
    },
    {
        "metric": "pct_female_headed_households_included_as_numeric_feature",
        "value": False,
    },
    {
...
unresolved_df.to_csv(OUTPUT_UNRESOLVED_VARIABLES, index=False, encoding="utf-8")
...
print("\n" + "=" * 72)
print("CLEAN CENSUS DIVISION HOUSEHOLD / FAMILY FEATURES 2021")
print("=" * 72)
...
for unresolved in UNRESOLVED_VARIABLES:
    print(f"- {unresolved['canonical_variable']}: {unresolved['status']}")
...
print(OUTPUT_UNRESOLVED_VARIABLES)
