...
# ============================================================
# Inspect Census Division Household / Family Features 2021
# ============================================================
#
# Purpose:
#   Inspect the 2021 Census Profile at census-division level for household
#   and family-structure variables needed by the SoVI-like census-division table.
#
...
#   The Canadian Census Profile may not provide a direct "female-headed
#   households" row in the same form as the original SoVI variable.
#
#   This inspection therefore searches for:
#       1. Average household size / average number of persons per household.
#       2. Female-parent lone-parent family rows.
#       3. Lone-parent family denominator rows.
#       4. Household and census-family rows that may support a derived proxy.
#
# This script does NOT create the final clean household/family table.
# It identifies candidates, coverage, value columns, symbols, and formulas
# that may be needed before choosing the cleaner logic.
#
# Expected raw source:
...
OUTPUT_FILE_INVENTORY = OUTPUT_DIR / "household_family_file_inventory_2021.csv"
OUTPUT_CD_INVENTORY = OUTPUT_DIR / "household_family_quebec_cd_inventory_2021.csv"
OUTPUT_BASE_JOIN = OUTPUT_DIR / "household_family_quebec_cd_base_join_check_2021.csv"
OUTPUT_CANDIDATE_CHARACTERISTICS = OUTPUT_DIR / "household_family_candidate_characteristics_2021.csv"
OUTPUT_TARGET_SUMMARY = OUTPUT_DIR / "household_family_target_characteristic_summary_2021.csv"
OUTPUT_TARGET_VALUES_LONG = OUTPUT_DIR / "household_family_target_values_long_2021.csv"
OUTPUT_TARGET_VALUES_WIDE = OUTPUT_DIR / "household_family_target_values_wide_2021.csv"
OUTPUT_DERIVED_FORMULA_AUDIT = OUTPUT_DIR / "household_family_derived_formula_audit_2021.csv"
OUTPUT_SYMBOL_COUNTS = OUTPUT_DIR / "household_family_symbol_counts_2021.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "household_family_inspection_summary_2021.csv"
...
# Broad inspection ranges. The exact IDs can vary by Census Profile hierarchy,
# so this section uses keyword matching plus context ranges.
HOUSEHOLD_FAMILY_CONTEXT_MIN_ID = 60
HOUSEHOLD_FAMILY_CONTEXT_MAX_ID = 220

HOUSEHOLD_FAMILY_KEYWORDS_ANY = [
    "household",
    "households",
    "household size",
...
HOUSEHOLD_FAMILY_KEYWORDS_EXCLUDE = [
    "language",
    "immigration",
    "mobility",
    "commuting",
...
# These are inspection targets, not final cleaner decisions.
# The final cleaner should use the inspection output to choose exact IDs.
HOUSEHOLD_FAMILY_TARGETS = [
    {
        "original_code": "AVGPERHH",
        "canonical_variable": "avg_people_per_household",
        "candidate_output_alias": "avg_household_size",
        "description": "Average number of persons per household",
        "target_family": "household_size",
        ...
        "sovi_role": "average_household_size_proxy",
        "notes": (
            "Original SoVI uses average number of persons per household. "
            "The cleaner should use the broad average household size row."
        ),
    },
    {
        "original_code": "PCTF_HH90",
        "canonical_variable": "pct_female_headed_households_candidate_rate",
        "candidate_output_alias": "pct_female_parent_lone_parent_family_rate",
        "description": "Candidate rate for female-headed households using female-parent lone-parent families",
        "target_family": "female_headed_household_proxy_rate",
        "preferred_name_contains_any": [
            "female parent",
            "female lone-parent",
            "female lone parent",
        ...
        "unit": "percent",
        "sovi_role": "candidate_female_headed_household_proxy",
        "notes": (
            "This may represent female-parent lone-parent families as a share of a "
            "family denominator, not necessarily as a share of all households. "
            "Review before using as final SoVI input."
        ),
    },
    {
        "original_code": "PCTF_HH90_DENOM",
        "canonical_variable": "total_lone_parent_families_candidate",
        "candidate_output_alias": "total_lone_parent_families",
        "description": "Candidate denominator for deriving female-parent share among lone-parent families",
        "target_family": "lone_parent_denominator",
        "preferred_name_contains_any": [
            "total - lone-parent",
            "total lone-parent",
        ...
        "symbol_column": "SYMBOL",
        "unit": "count",
        "sovi_role": "candidate_lone_parent_denominator",
        "notes": (
            "Possible denominator for a derived family-structure proxy. "
            "Not itself the final SoVI variable."
        ),
    },
    {
        "original_code": "PCTF_HH90_HH_DENOM",
        "canonical_variable": "total_private_households_candidate",
        "candidate_output_alias": "total_private_households",
        "description": "Candidate denominator for deriving female-parent families as a share of private households",
        "target_family": "household_denominator",
        "preferred_name_contains_any": [
        ...
        "symbol_column": "SYMBOL",
        "unit": "count",
        "sovi_role": "candidate_private_household_denominator",
        "notes": (
            "Possible denominator if the final proxy is female-parent lone-parent families "
            "as a share of all private households."
        ),
    },
]
...
def is_household_family_candidate(row: pd.Series) -> bool:
    ...
    in_context_range = (
        char_id is not None
        and HOUSEHOLD_FAMILY_CONTEXT_MIN_ID <= char_id <= HOUSEHOLD_FAMILY_CONTEXT_MAX_ID
    )

    keyword_match = any(keyword.lower() in name for keyword in HOUSEHOLD_FAMILY_KEYWORDS_ANY)
    excluded = any(keyword.lower() in name for keyword in HOUSEHOLD_FAMILY_KEYWORDS_EXCLUDE)
...
def target_matches_row(target: dict, row: pd.Series) -> tuple[bool, str]:
    ...
    if any(term in name for term in avoid_terms):
        return False, ""
...
print("\nCensus Profile CD household/family inspection")
print("Raw file:", RAW_CSV.relative_to(DATA_DIR))
print("Raw encoding selected:", raw_encoding)
print(
    "Household/family context ID range:",
    f"{HOUSEHOLD_FAMILY_CONTEXT_MIN_ID}–{HOUSEHOLD_FAMILY_CONTEXT_MAX_ID}",
)
...
for chunk_idx, chunk in enumerate(
    ...
    candidate_mask = qc.apply(is_household_family_candidate, axis=1)
    ...
    if not candidates.empty:
        ...
        for _, char_row in characteristic_candidates.iterrows():
            for target in HOUSEHOLD_FAMILY_TARGETS:
...
if not target_values_long.empty:
    wide_for_formula = target_values_long.pivot_table(
        index=[
            "census_division_dguid",
            "census_division_code",
            "census_division_name_profile",
        ],
        columns="candidate_output_alias",
        values="value_numeric",
        aggfunc="first",
    ).reset_index()

    wide_for_formula.columns.name = None

    # Possible formula 1:
    #   female_parent_lone_parent_families / total_lone_parent_families
    if (
        "pct_female_parent_lone_parent_family_rate" in wide_for_formula.columns
        and "total_lone_parent_families" in wide_for_formula.columns
    ):
        # In many Census Profile tables, the female-parent row may already
        # provide a rate, but this audit records it explicitly for review.
        derived_formula_rows.append(
            {
                "candidate_formula": "female_parent_lone_parent_family_rate_from_profile",
                "description": (
                    "Uses the published C10_RATE_TOTAL on the female-parent row. "
                    "Likely percent of lone-parent families with a female parent, not "
                    "percent of all households."
                ),
                "available": True,
                "recommended_as_default_without_review": False,
            }
        )

    # Possible formula 2:
    #   count female-parent lone-parent families / total private households
    #
    # This inspection stores the necessary candidate rows, but the cleaner should
    # only use this formula after confirming the correct numerator and denominator IDs.
    if (
        "pct_female_parent_lone_parent_family_rate" in wide_for_formula.columns
        and "total_private_households" in wide_for_formula.columns
    ):
        derived_formula_rows.append(
            {
                "candidate_formula": "female_parent_lone_parent_families_per_100_private_households",
                "description": (
                    "Potential closer proxy to female-headed households if the female-parent "
                    "row count and total private-household denominator are confirmed."
                ),
                "available": True,
                "recommended_as_default_without_review": False,
            }
...
derived_formula_audit = pd.DataFrame(
    derived_formula_rows
    if derived_formula_rows
    else [
        {
            "candidate_formula": "",
            "description": "No complete derived formula candidates were automatically confirmed.",
            "available": False,
            "recommended_as_default_without_review": False,
        }
    ]
...
summary = pd.DataFrame(
    [
        ...
        },
        {
            ...
            "value": (
                "Review household_family_target_characteristic_summary_2021.csv before generating cleaner. "
                "Average household size may be direct, but female-headed households likely require "
                "an explicit proxy decision."
            ),
        },
    ]
)
...
print("\n" + "=" * 72)
print("CENSUS DIVISION HOUSEHOLD/FAMILY INSPECTION SUMMARY")
print("=" * 72)
...
print("\nHousehold/family candidate characteristics:")
